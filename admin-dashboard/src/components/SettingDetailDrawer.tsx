...
function humanizeSettingKey(key: string): string {
  const last = key.split(".").filter(Boolean).slice(-1)[0] ?? key;
  return last
    .replace(/_/g, " ")
...
export function SettingDetailDrawer(props: {
  ...
  const impactKeys = Object.keys(impacts).filter((k) => Boolean((impacts as any)[k])) as Array<keyof typeof impacts>;
      ...
      <div className="absolute right-0 top-0 flex h-full w-full max-w-2xl flex-col bg-white shadow-2xl" role="dialog" aria-modal="true">
        <div className="flex items-start justify-between gap-3 border-b border-zinc-200 px-4 py-3">
          <div className="min-w-0">
            ...
            {(hint?.title ?? (settingKey ? humanizeSettingKey(settingKey) : null)) ? (
              <div className="mt-1 text-sm font-semibold text-zinc-900">{hint?.title ?? (settingKey ? humanizeSettingKey(settingKey) : "")}</div>
            ) : null}
          ...
          <div className="flex items-center gap-2">
            ...
            <Button type="button" variant="ghost" className="rounded-lg border border-zinc-200 bg-white px-2 py-1 text-xs font-semibold text-zinc-700 hover:bg-zinc-50" onClick={onClose}>Close</Button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
            ...
            <div className="mt-2 whitespace-pre-wrap text-sm text-zinc-700">{hint?.details ?? entry?.description ?? "No documentation for this setting yet."}</div>
          ...
          {hint?.related?.length ? (
            <div className="mt-4 rounded-2xl border border-zinc-200 bg-white p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {hint.related.map((k) => (
                  <Button key={k} type="button" variant="ghost" onClick={() => onNavigateToKey?.(k)} className="rounded-full border border-zinc-200 bg-white px-3 py-1 text-xs font-semibold text-zinc-700 hover:bg-zinc-50">{k}</Button>
                ))}
              </div>
            </div>
          ) : null}
