...
export const SETTING_HINTS: Record<string, SettingHint> = {
  ...
  "admin.diagnostics.read_source_default": {
    title: "Diagnostics read source",
    details: "Controls where the admin dashboard should read diagnostic data from by default: direct PostgREST table reads (requires admin RLS) vs Edge Functions.",
    examples: [
      { scenario: "auto", effect: "Try direct DB reads first for speed; fall back to Edge Functions if blocked." },
      { scenario: "direct", effect: "Force direct table reads only (fastest when RLS is correct; fails loudly if not)." },
      { scenario: "edge", effect: "Force Edge Functions only (most compatible; slightly more latency)." },
    ],
    recommended: "auto",
    recommended_why: "Best default for production: fast when direct reads work, but still resilient if an admin RLS policy is missing.",
    impacts: { ux: "admin responsiveness", performance: "latency", ops: "compatibility" },
    caution: "Use 'direct' only if you're sure admin RLS policies allow the relevant tables; otherwise diagnostics pages may error.",
  },

  "admin.diagnostics.read_source_overrides": {
    title: "Diagnostics read source overrides",
    details: "Per-section override for diagnostics read source. Value is a JSON object mapping section name -> 'auto' | 'direct' | 'edge'.",
    examples: [
      { scenario: "{ openrouter_request_log: 'edge' }", effect: "Force the OpenRouter Requests page to always use the Edge Function." },
      { scenario: "{ job_run_log: 'direct' }", effect: "Force Jobs/Logs reads to come directly from tables." },
    ],
    recommended: {},
    recommended_why: "Leave empty unless you need to pin a specific page to Edge or Direct while debugging.",
    impacts: { ops: "targeted overrides", performance: "latency vs compatibility" },
  },
