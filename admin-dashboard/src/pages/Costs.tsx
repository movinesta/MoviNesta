...
export default function Costs() {
  ...
  const dailyRows = useMemo(() => {
    ...
    // Be defensive: network/proxy errors or partial responses can yield null/undefined rows.
    return raw
      .filter(Boolean)
    ...
    <div className="space-y-6">
      ...
      <Card title="Tokens by day (from job_run_log)">
        ...
        <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-900/30 p-3 text-xs text-zinc-300">
          Total records: <span className="font-mono">{fmtInt(dailyRows.length)}</span>
        </div>
      </Card>
    </div>
  );
