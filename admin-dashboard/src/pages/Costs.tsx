...
export default function Costs() {
  ...
  const refreshCaches = useMutation({
    mutationFn: async () => {
      const base_url = (assistantSettingsQ.data as any)?.assistant_settings?.openrouter_base_url ?? null;
      return refreshOpenRouterCaches({ base_url, refresh_parameters: false });
    },
    onSuccess: () => {
      creditsQ.refetch();
      keyQ.refetch();
    ...
    <div className="space-y-6">
      ...
      <Card
        ...
        right={
          <Button
            variant="secondary"
            onClick={() => refreshCaches.mutate()}
            disabled={refreshCaches.isPending}
          >
            {refreshCaches.isPending ? "Refreshing…" : "Refresh caches"}
          </Button>
        }
      >
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
