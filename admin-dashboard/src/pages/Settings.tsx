    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-3xl rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl">
        <div className="flex items-start justify-between gap-3">
          ...
          <button className="rounded-lg px-2 py-1 text-xs text-zinc-500 hover:bg-zinc-100" onClick={props.onClose}>
            ✕
          </button>
        </div>
...
const categoryChangedCounts = useMemo(() => {
  ...
  const keysFiltered = useMemo(() => {
    ...
    return out;
  }, [registry, search, category, scope, favoritesOnly, favorites, changedOnly, rowByKey]);
  ...
  const mutExport = useMutation({
    ...
    onError: (e: any) => {
      toast.push({ variant: "error", title: "Export failed", message: e?.message ?? String(e) });
    },
  });
  ...
  const mutImportPreview = useMutation({
    ...
    onError: (e: any) => {
      toast.push({ variant: "error", title: "Preview failed", message: e?.message ?? String(e), durationMs: 6000 });
    },
  });
  ...
  const mutImportApply = useMutation({
    ...
    onError: (e: any) => {
      toast.push({ variant: "error", title: "Import failed", message: e?.message ?? String(e), durationMs: 6000 });
    },
  });
  ...
  const mutSave = useMutation({
    ...
    onError: (e: any) => {
      toast.push({ variant: "error", title: "Save failed", message: e?.message ?? String(e), durationMs: 6000 });
    },
  });
  ...
  const mutCleanupRedundant = useMutation({
    ...
    onError: (e: any) => {
      toast.push({ variant: "error", title: "Cleanup failed", message: e?.message ?? String(e) });
    },
  });
  ...
  const mutHistory = useMutation({
    ...
    onError: (e: any) => {
      toast.push({ variant: "error", title: "History failed", message: e?.message ?? String(e) });
    },
  });
  ...
  const mutRollback = useMutation({
    ...
    onError: (e: any) => {
      toast.push({ variant: "error", title: "Rollback failed", message: e?.message ?? String(e) });
    },
  });
  ...
  <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
    ...
    <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
            ...
            <Button
              onClick={() => {
                ...
                try {
                  ...
                  toast.push({ variant: "error", title: "Invalid input", message: e?.message ?? String(e) });
                  return;
                }
                mutSave.mutate();
              }}
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-12">
          ...
          <div className="md:col-span-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="mb-1 text-xs font-medium text-zinc-600">Scope</div>
                <select
                  value={scope}
                            ...
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <button
                                  ...
                                  title="Copy key"
                                  aria-label="Copy key"
                                  className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-zinc-200 bg-white text-[12px] font-semibold text-zinc-500 shadow-sm hover:text-zinc-700"
                                >
                                  ⧉
                                </button>
                                <button
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      try {
                                        ...
                                        toast.push({ variant: "error", title: "Invalid JSON", message: e?.message ?? String(e) });
                                      }
                                    }}
                                  >
                                    Format JSON
      ...
      <SettingDetailDrawer
        ...
        onNavigateToKey={(k) => jumpToKey(k, { openDetails: true })}
      />
