...
import { ProviderPolicyEditor, type ProviderOption } from "../components/ProviderPolicyEditor";
...
function extractEndpoints(payload: any): any[] {
  if (!payload) return [];
  if (Array.isArray(payload)) return payload;

  const candidates: any[] =
    (Array.isArray(payload?.data) ? payload.data : null) ??
    (Array.isArray(payload?.endpoints) ? payload.endpoints : null) ??
    ...
    (Array.isArray(payload?.endpoints?.data) ? payload.endpoints.data : null) ??
    [];

  return candidates;
}

function extractProviderSlug(ep: any): string | null {
  if (!ep || typeof ep !== "object") return null;
  const raw = (ep as any).provider ?? (ep as any).provider_name ?? (ep as any).providerName ?? null;
  if (typeof raw === "string" && raw.trim()) return raw.trim();
  if (raw && typeof raw === "object") {
    const slug = (raw as any).slug ?? (raw as any).id ?? (raw as any).name ?? null;
    if (typeof slug === "string" && slug.trim()) return slug.trim();
  }
  const name = (ep as any).name ?? (ep as any).provider_id ?? null;
...
const PARAM_FIELDS: ParamField[] = [
  ...
  { key: "reasoning", label: "Reasoning (JSON)", type: "json" },

  // Native tool calling (Responses API function_call / function_call_output)
  { key: "native_tool_calling", label: "Native tool calling", type: "boolean" },
  { key: "native_tool_max_loops", label: "Native tool max loops", type: "number", min: 1, max: 12, step: 1 },
  {
    key: "native_tool_max_calls_per_loop",
    label: "Native tool max calls per loop",
    type: "number",
    min: 1,
    max: 20,
    step: 1,
  ...
  { key: "stream_passthrough", label: "Stream passthrough (SSE)", type: "boolean" },
  ...
  // Plugins
  { key: "response_healing", label: "Response healing (plugin)", type: "boolean" },
  { key: "web_search", label: "Web search (plugin)", type: "boolean" },
  { key: "web_search_mode", label: "Web search mode (off/auto/always)", type: "text" },
  { key: "web_max_results", label: "Web search max results", type: "number", min: 1, max: 10, step: 1 },
  { key: "web_engine", label: "Web search engine (native/exa)", type: "text" },
  { key: "web_search_prompt", label: "Web search prompt", type: "text" },
...
export default function Assistant() {
  const [circuitActionMsg, setCircuitActionMsg] = useState<string | null>(null);
  const [circuitActionErr, setCircuitActionErr] = useState<string | null>(null);
  const [circuitKeepDays, setCircuitKeepDays] = useState<number>(14);
  ...
  const [routingTestSimulateAdvanced, setRoutingTestSimulateAdvanced] = useState<boolean>(false);
  ...
  const [routingLogCursor, setRoutingLogCursor] = useState<string | null>(null);
  const [routingLogHistory, setRoutingLogHistory] = useState<(string | null)[]>([]);
  ...
  const routingLogQ = useQuery({
    queryKey: [
      ...
      { limit: routingLogLimit, cursor: routingLogCursor, requestId: routingLogRequestId, fn: routingLogFn },
    ],
    queryFn: () =>
      getOpenRouterRequestLog({
        cursor: routingLogCursor,
  ...
  const runRoutingTest = useMutation({
    mutationFn: (payload: { prompt?: string; mode?: "current" | "auto" | "fallback"; simulate_advanced_params?: boolean }) => testAssistantRouting(payload),
    onSuccess: (resp) => {
  ...
  const resetCircuit = useMutation({
    mutationFn: (model?: string | null) => resetOpenRouterCircuit(model ?? null),
    onSuccess: (resp: any, model) => {
      setCircuitActionErr(null);
      const affected = Number(resp?.affected ?? resp?.data?.affected ?? 0);
      const label = model && String(model).trim() ? String(model).trim() : "all models";
      setCircuitActionMsg(`Circuit reset: ${label} (${affected} row${affected === 1 ? "" : "s"})`);
      healthQ.refetch();
    },
    onError: (err: any) => {
      setCircuitActionMsg(null);
      setCircuitActionErr(err?.message ?? "Failed to reset circuit");
    },
  });

  const cleanupCircuits = useMutation({
    mutationFn: (keepDays: number) => cleanupOpenRouterCircuits({ keep_days: keepDays }),
    onSuccess: (resp: any) => {
      setCircuitActionErr(null);
      const deleted = Number(resp?.deleted ?? resp?.data?.deleted ?? 0);
      const keepDays = Number(resp?.keep_days ?? resp?.data?.keep_days ?? 14);
      setCircuitActionMsg(`Circuit history cleaned: deleted ${deleted} row${deleted === 1 ? "" : "s"} (kept ${keepDays}d)`);
      healthQ.refetch();
    },
    onError: (err: any) => {
      setCircuitActionMsg(null);
      setCircuitActionErr(err?.message ?? "Failed to cleanup circuit history");
    },
  });
  ...
  const routingNextCursor = (routingLogQ.data as any)?.next_cursor ?? null;
  ...
  const providerOptions: ProviderOption[] = useMemo(() => {
    const payload = endpointsData?.payload ?? null;
    const endpoints = extractEndpoints(payload);
    const map = new Map<string, ProviderOption>();
    for (const ep of endpoints) {
      const slug = extractProviderSlug(ep);
      if (!slug) continue;

      let name: string | null = null;
      const rawProv = (ep as any).provider ?? (ep as any).provider_name ?? null;
      if (rawProv && typeof rawProv === "object") {
        const n = (rawProv as any).name ?? (rawProv as any).display_name ?? null;
        if (typeof n === "string" && n.trim()) name = n.trim();
      }
      if (!name && typeof (ep as any).provider_name === "string" && (ep as any).provider_name.trim()) {
        name = String((ep as any).provider_name).trim();
      }
  ...
  const FIELD_ID_BY_KEY: Record<string, string> = {
    ...
    "assistant_settings.params.stream_passthrough": "field-param-stream_passthrough",
    "assistant_settings.params.response_healing": "field-param-response_healing",
    "assistant_settings.params.web_search": "field-param-web_search",
    "assistant_settings.params.web_search_mode": "field-param-web_search_mode",
    "assistant_settings.params.web_max_results": "field-param-web_max_results",
    "assistant_settings.params.web_engine": "field-param-web_engine",
    "assistant_settings.params.web_search_prompt": "field-param-web_search_prompt",
  ...
  useEffect(() => {
    // Changing filters should restart pagination.
    setRoutingLogCursor(null);
    setRoutingLogHistory([]);
    setRoutingLogSelectedId(null);
  }, [routingLogLimit, routingLogFn, routingLogRequestId]);
    ...
    if (tab === "diagnostics") {
      ...
      return (
        <>
          ...
          {(health?.aiFailureRollup?.codes?.length ?? 0) > 0 || (health?.aiFailureRollup?.models?.length ?? 0) > 0 ? (
            <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
              <Card>
                <div className="mb-2 text-sm font-semibold">AI error rollup (72h)</div>
                <div className="text-xs text-zinc-500">Top error codes captured by <code>assistant_failures</code>.</div>
                <Table>
                  <thead>
                    <tr>
                      <Th>Code</Th>
                  ...
                  <tbody>
                    {(health?.aiFailureRollup?.codes ?? []).map((r: any) => (
                      <tr key={r.code} className="border-t border-zinc-100">
                        <Td className="font-mono text-xs">{r.code}</Td>
                        <Td className="text-right">{fmtInt(Number(r.count ?? 0))}</Td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
              ...
              <Card>
                <div className="mb-2 text-sm font-semibold">Most affected models (72h)</div>
                <div className="text-xs text-zinc-500">Derived from <code>assistant_failures.details.ai.context.model</code>.</div>
                <Table>
                  <thead>
                    <tr>
                      <Th>Model</Th>
                  ...
                  <tbody>
                    {(health?.aiFailureRollup?.models ?? []).map((r: any) => (
          ...
          {(health?.openrouterCircuits ?? []).length ? (
            <div className="mt-6">
              <Card>
                <div className="mb-2 text-sm font-semibold">OpenRouter circuits (currently skipped models)</div>
                <div className="text-xs text-zinc-500">
                  Models listed here are temporarily skipped by the Edge Functions circuit breaker to reduce repeated 429/5xx storms.
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <Button
                    ...
                    onClick={() => {
                      setCircuitActionMsg(null);
                      setCircuitActionErr(null);
                      resetCircuit.mutate(null);
                    }}
                    disabled={resetCircuit.isPending || cleanupCircuits.isPending}
                  >
                    Reset all
                  </Button>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      min={1}
                      max={365}
                      value={String(circuitKeepDays)}
                      onChange={(e) => {
                        const n = Number(e.target.value);
                        if (!Number.isFinite(n)) return;
                        setCircuitKeepDays(Math.max(1, Math.min(365, Math.trunc(n))));
                      }}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setCircuitActionMsg(null);
                      setCircuitActionErr(null);
                      cleanupCircuits.mutate(circuitKeepDays);
                    }}
                    disabled={resetCircuit.isPending || cleanupCircuits.isPending}
                  >
                    Cleanup history
                  </Button>
                  {resetCircuit.isPending || cleanupCircuits.isPending ? (
                    <span className="text-xs text-zinc-500">Working…</span>
                  ) : null}
                </div>
                {circuitActionMsg ? <div className="mt-2 text-xs text-zinc-700">{circuitActionMsg}</div> : null}
                {circuitActionErr ? <div className="mt-2 text-xs text-red-600">{circuitActionErr}</div> : null}
                <Table>
                  <thead>
                    <tr>
                      <Th>Model</Th>
                  ...
                  <tbody>
                    {(health?.openrouterCircuits ?? []).map((r: any) => (
                      <tr key={r.model} className="border-t border-zinc-100">
                        <Td className="font-mono text-xs">{r.model}</Td>
                        <Td>{fmtTs(r.openUntil)}</Td>
                        <Td className="text-right">{fmtInt(Number(r.failureStreak ?? 0))}</Td>
                        <Td className="text-right">{r.lastStatus ?? ""}</Td>
                        ...
                        <Td className="text-right">
                          <Button
                            ...
                            onClick={() => {
                              setCircuitActionMsg(null);
                              setCircuitActionErr(null);
                              resetCircuit.mutate(String(r.model));
                            }}
                            disabled={resetCircuit.isPending || cleanupCircuits.isPending}
                          >
                            Reset
              ...
              {routingLogQ.isLoading ? (
                ...
                <div className="mt-4 flex items-center justify-between gap-2">
                  <Button
                    type="button"
                    variant="secondary"
                    className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-xs disabled:opacity-50"
                    onClick={() => {
                      setRoutingLogCursor(null);
                      setRoutingLogHistory([]);
                      setRoutingLogSelectedId(null);
                    }}
                    disabled={!routingLogCursor && routingLogHistory.length === 0}
                  >
                    First page
                  </Button>

                  <div className="ml-auto text-[11px] text-zinc-500">
                    Applied: limit <span className="font-mono">{routingLogLimit}</span>
                    {routingLogFn.trim() ? (
                      <>
                        {" "}· fn <span className="font-mono">{routingLogFn.trim()}</span>
                      </>
                    ) : null}
                    {routingLogRequestId.trim() ? (
                      <>
                        {" "}· request <span className="font-mono">{routingLogRequestId.trim()}</span>
                      </>
                    ) : null}
                    {routingLogCursor ? (
                      <>
                        {" "}· cursor <span className="font-mono">{routingLogCursor}</span>
                      </>
                    ) : null}
                  </div>
                  ...
                  <Button
                    ...
                    className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-xs disabled:opacity-50"
                    onClick={() => {
                      const prev = routingLogHistory[routingLogHistory.length - 1] ?? null;
                      setRoutingLogHistory((h) => h.slice(0, -1));
                      setRoutingLogCursor(prev);
                      setRoutingLogSelectedId(null);
                    }}
                    disabled={routingLogHistory.length === 0}
                  >
                    Prev
                  </Button>
                  ...
                  <Button
                    ...
                    className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-xs disabled:opacity-50"
                    onClick={() => {
                      if (!routingNextCursor) return;
                      setRoutingLogHistory((h) => [...h, routingLogCursor]);
                      setRoutingLogCursor(routingNextCursor);
                      setRoutingLogSelectedId(null);
                    }}
                    disabled={!routingNextCursor}
                  >
                    Next
                  </Button>
                </div>
            ...
            <div className="mt-4">
              <Card>
                  ...
                  <div className="flex items-end md:col-span-1">
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        setRoutingTestResult(null);
                        try {
                          await runRoutingTest.mutateAsync({ prompt: routingTestPrompt, mode: routingTestMode, simulate_advanced_params: routingTestSimulateAdvanced });
                        } catch {
                          // handled in onError
                        }
                      }}
                ...
                <label className="mt-3 flex items-center gap-2 text-xs text-zinc-600">
                  <input
                    type="checkbox"
                    checked={routingTestSimulateAdvanced}
                    onChange={(e) => setRoutingTestSimulateAdvanced(e.target.checked)}
                  />
                  Simulate advanced params (tools/tool_choice) to test provider <span className="font-mono">require_parameters</span>
                </label>
                      ...
                      <div className="mt-2 space-y-2 text-sm text-zinc-700">
                        ...
                        <div>
                          ...
                          <span className="font-mono text-xs">
                            {routingTestResult.test.requireParameters === null || typeof routingTestResult.test.requireParameters === "undefined"
                              ? "—"
                              : String(routingTestResult.test.requireParameters)}
                          </span>
                          <span className="ml-2 text-xs text-zinc-500">
                            (source: {routingTestResult.test.requireParametersSource ?? "—"})
                          </span>
                        </div>
                        <div>
                          Base URL: <span className="font-mono text-xs">{routingTestResult.test.baseUrl ?? "—"}</span>
                        </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      ...
                      <div className="rounded-lg border border-zinc-200 bg-white p-3 md:col-span-2">
                        ...
                        <div className="mt-3 rounded-lg border border-zinc-200 bg-white p-3">
                          <div className="mb-2 flex items-center justify-between gap-2 text-[11px] font-semibold text-zinc-600">
                            <div>Provider policy editor (recommended)</div>
                            <HintIcon
                              title="Details"
                              onClick={() => openHint("behavior.router.policy.provider", { scroll: false })}
                            />
                          </div>

                          <ProviderPolicyEditor
                            value={getIn(parsedBehavior.value, ["router", "policy", "provider"], null)}
                            providerOptions={providerOptions}
                            onChange={(next) =>
                              updateBehaviorJson((b) => setIn(b, ["router", "policy", "provider"], next))
                            }
                          />

                          <div className="mt-2 text-[11px] text-zinc-500">
                            Uses OpenRouter&apos;s <span className="font-mono">provider</span> routing fields (only/ignore/order, etc.).
                          </div>
                        </div>
                    ...
                    if (field.key === "verbosity") {
                      return (
                        <label key={field.key} id={`field-param-${field.key}`} className="flex flex-col gap-1 text-sm">
                          <span className="flex items-center justify-between gap-2 text-xs font-semibold text-zinc-600">
                            <span>{field.label}</span>
                            {hasParamHint ? <HintIcon title="Details" onClick={() => openHint(paramHintKey)} /> : null}
                          </span>
                          <select
                            value={value}
                            onChange={(e) =>
                      ...
                      return (
                        <label key={field.key} id={`field-param-${field.key}`} className="flex flex-col gap-1 text-sm">
                            ...
                            onChange={(e) => {
                              ...
                              const next =
                                v === ""
                                  ...
                                  : v === "__custom__"
                                    ? value
                                    : JSON.stringify({ effort: v }, null, 2);
                              setSettingsForm((prev) =>
                                prev ? { ...prev, params: { ...prev.params, [field.key]: next } } : prev,
                              );
                            }}
                            className="rounded-lg border border-zinc-200 px-3 py-2 text-sm"
                          >
                          ...
                          <textarea
                            ...
                            className="h-24 w-full rounded-lg border border-zinc-200 px-3 py-2 text-xs font-mono"
                          />
                        </label>
                      );
                    }
...
function RoutingLogDetailsDrawer(props: { open: boolean; row: OpenRouterRequestLogRow | null; onClose: () => void }) {
  ...
  const generationStats = (() => {
    const meta: any = r.meta as any;
    if (!meta || typeof meta !== "object") return null;
    const gs = meta.generation_stats ?? meta.generationStats ?? null;
    if (!gs || typeof gs !== "object") return null;
    return gs as Record<string, any>;
  })();

  const generationText = (() => {
    if (!generationStats) return "";
    try {
      return JSON.stringify(generationStats, null, 2);
    } catch {
      return String(generationStats ?? "{}");
    }
  })();
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-xl border-l border-zinc-200 bg-white p-5 shadow-xl">
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between gap-2">
            <div className="text-xs font-medium text-zinc-500">Generation stats</div>
            {generationStats ? (
              <CopyButton text={generationText} label="Copy generation JSON" className="h-8 px-2 py-1 text-xs" />
            ) : (
              <div className="text-[11px] text-zinc-500">Not backfilled yet</div>
            )}
          </div>
          ...
          {generationStats ? (
            <div className="mt-2 rounded-2xl border border-zinc-200 bg-white p-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <div className="text-[11px] font-medium text-zinc-500">Total cost (USD)</div>
                  <div className="font-mono text-xs text-zinc-700">{String(generationStats.total_cost ?? generationStats.totalCost ?? "—")}</div>
                </div>
                <div>
                  <div className="text-[11px] font-medium text-zinc-500">Provider</div>
                  <div className="font-mono text-xs text-zinc-700">{String(generationStats.provider_name ?? generationStats.providerName ?? generationStats.provider ?? "—")}</div>
                </div>
                <div>
                  <div className="text-[11px] font-medium text-zinc-500">Native prompt tokens</div>
                  <div className="font-mono text-xs text-zinc-700">{String(generationStats.native_tokens_prompt ?? generationStats.nativeTokensPrompt ?? generationStats.tokens_prompt ?? generationStats.tokensPrompt ?? "—")}</div>
                </div>
                <div>
                  <div className="text-[11px] font-medium text-zinc-500">Native completion tokens</div>
                  <div className="font-mono text-xs text-zinc-700">{String(generationStats.native_tokens_completion ?? generationStats.nativeTokensCompletion ?? generationStats.tokens_completion ?? generationStats.tokensCompletion ?? "—")}</div>
                </div>
              </div>

              <details className="mt-3">
                <summary className="cursor-pointer select-none text-xs font-medium text-zinc-600">Raw generation JSON</summary>
                <pre className="mt-2 max-h-[18vh] overflow-auto rounded-2xl border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-700">{generationText}</pre>
              </details>
            </div>
          ) : (
            <div className="mt-2 rounded-2xl border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-600">
              This request has no cached generation stats yet. Run the
              <span className="mx-1 font-mono">openrouter-generation-backfill</span> job (or wait for its cron schedule)
              to populate cost/native token counts from OpenRouter&apos;s generation API.
            </div>
          )}
        </div>
