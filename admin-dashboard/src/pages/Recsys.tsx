...
import {
  ...
  type SwipeIngestHealthResp,
  type SwipeIngestHourlyRow,
  type SwipeIngestIssueTotalRow,
} from "../lib/api";
...
import { RecsysKnobsEditor } from "../components/RecsysKnobsEditor";
...
export default function Recsys() {
  ...
  const qJoin = useQuery({
    queryKey: useMemo(() => ["rec_join_integrity", days], [days]),
    queryFn: async () => getRecJoinIntegrity({ days, include_by_event: true }),
    staleTime: 30_000,
    refetchInterval: 60_000,
    enabled: tab === "health",
  });
  ...
  const qSwipeIngest = useQuery({
    queryKey: useMemo(() => ["swipe_ingest_health", 72], []),
    queryFn: async () => getSwipeIngestHealth({ hours: 72, top_n: 12 }),
    staleTime: 30_000,
    refetchInterval: 60_000,
    enabled: tab === "health",
  });
  ...
  const swipeIngest = (qSwipeIngest.data as any)?.ok ? (qSwipeIngest.data as SwipeIngestHealthResp) : null;
  const swipeHourly = (swipeIngest?.hourly ?? []) as SwipeIngestHourlyRow[];
  const swipeIssues = (swipeIngest?.issues ?? []) as SwipeIngestIssueTotalRow[];
  const latestSwipe = swipeHourly.length ? swipeHourly[0] : null;
  const swipeHourlyChrono = useMemo(() => swipeHourly.slice().reverse(), [swipeHourly]);
  ...
  const joinDaily = (qJoin.data?.daily ?? []) as RecJoinIntegrityRow[];
  const joinByEvent = (qJoin.data?.by_event ?? []) as RecJoinIntegrityByEventRow[];
  const latestJoin = joinDaily[0];
  const latestJoinDay = latestJoin?.day ?? null;
  const latestJoinByEvent = useMemo(() => {
    if (!latestJoinDay) return [];
    return joinByEvent
      .filter((r) => (r as any).day === latestJoinDay)
      .slice()
      .sort((a, b) => Number((b as any).events_without_impression ?? 0) - Number((a as any).events_without_impression ?? 0))
      .slice(0, 12);
  }, [joinByEvent, latestJoinDay]);
    ...
    <div className="space-y-4">
      ...
      {tab === "health" ? (
        <>
          ...
          {qJoin.isError ? <ErrorBox title="Failed to load join integrity" error={qJoin.error as any} /> : null}
          {qSwipeIngest.isError ? <ErrorBox title="Failed to load swipe ingest health" error={qSwipeIngest.error as any} /> : null}
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <div className="text-sm font-semibold text-zinc-900">Join integrity (impressions → outcomes)</div>
                <div className="text-xs text-zinc-500">
                  Detects disconnects between rec_impressions and media_events using (user_id, served_dedupe_key).
                </div>
              </div>
              <div className="text-xs text-zinc-500">
                Window: {qJoin.data?.since ? `${qJoin.data.since} → today` : qJoin.isLoading ? "Loading…" : "—"}
              </div>
            </div>

            <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-5">
              <StatCard
                title="Unjoinable rate"
                value={latestJoin ? fmtPct((latestJoin as any).event_unjoinable_rate) : "—"}
                subtitle={latestJoin ? `Day: ${latestJoin.day}` : undefined}
              />
              <StatCard
                title="Events w/o impression"
                value={latestJoin ? fmtInt((latestJoin as any).events_without_impression) : "—"}
                subtitle="Join gaps"
              />
              <StatCard
                title="Missing served key"
                value={latestJoin ? fmtInt((latestJoin as any).events_missing_served_dedupe_key) : "—"}
                subtitle="Cannot link to impression"
              />
              <StatCard
                title="Invalid served key"
                value={latestJoin ? fmtInt((latestJoin as any).events_invalid_served_dedupe_key) : "—"}
                subtitle="Format mismatches"
              />
              <StatCard
                title="Missing rec_request_id"
                value={latestJoin ? fmtInt((latestJoin as any).events_missing_rec_request_id) : "—"}
                subtitle="Identity drift"
              />
            </div>

            <div className="mt-4 h-56">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={joinDaily.slice().reverse()}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="day" tick={{ fontSize: 12 }} />
                  <YAxis tick={{ fontSize: 12 }} domain={[0, 1]} />
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="event_unjoinable_rate" name="Unjoinable rate" dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>

            {latestJoinByEvent.length ? (
              <div className="mt-4">
                <div className="text-xs font-medium text-zinc-700">Top unjoinable event types (latest day)</div>
                <Table className="mt-2">
                  <thead>
                    <tr>
                      ...
                      <Th className="text-right">Without impression</Th>
                      <Th className="text-right">Unjoinable</Th>
                    </tr>
                  </thead>
                  <tbody>
                    {latestJoinByEvent.map((r) => (
                      <tr key={`${r.day}_${r.event_type}_${r.source ?? ""}`}>
                        <Td>{(r as any).event_type}</Td>
                        <Td>{(r as any).source ?? "—"}</Td>
                        <Td className="text-right">{fmtInt((r as any).events)}</Td>
                        <Td className="text-right">{fmtInt((r as any).events_without_impression)}</Td>
                        <Td className="text-right">{fmtPct((r as any).event_unjoinable_rate)}</Td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <div className="text-sm font-semibold text-zinc-900">Swipe ingest health (sampled rollups)</div>
                <div className="text-xs text-zinc-500">Edge ingestion outcomes + issue codes aggregated per hour (best-effort, sampled to control write volume).</div>
              </div>
              <div className="text-xs text-zinc-500">
                Window: {swipeIngest?.since ? `${swipeIngest.since} → now` : qSwipeIngest.isLoading ? "Loading…" : "—"}
              </div>
            </div>
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-5">
              <StatCard
                title="Requests"
                value={latestSwipe ? fmtInt((latestSwipe as any).requests) : "—"}
                subtitle={latestSwipe ? `Bucket: ${(latestSwipe as any).bucket_start}` : undefined}
              />
              <StatCard
                title="Accepted"
                value={latestSwipe ? fmtInt((latestSwipe as any).accepted_events) : "—"}
                subtitle="Events"
              />
              <StatCard
                title="Rejected"
                value={latestSwipe ? fmtInt((latestSwipe as any).rejected_events) : "—"}
                subtitle={latestSwipe ? `Rate: ${fmtPct((latestSwipe as any).rejection_rate)}` : undefined}
              />
              <StatCard
                title="Retry"
                value={latestSwipe ? fmtInt((latestSwipe as any).retry_events) : "—"}
                subtitle={latestSwipe ? `Rate: ${fmtPct((latestSwipe as any).retry_rate)}` : undefined}
              />
              <StatCard
                title="Sample rate"
                value={latestSwipe ? String((latestSwipe as any).sample_rate ?? "—") : "—"}
                subtitle={latestSwipe?.updated_at ? `Updated: ${fmtDate((latestSwipe as any).updated_at)}` : undefined}
              />
            </div>

            <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-3">
              <div className="lg:col-span-2">
                <div className="text-xs font-medium text-zinc-700">Rejection / retry rates (hourly)</div>
                <div className="mt-2 h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={swipeHourlyChrono}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="bucket_start" tick={{ fontSize: 10 }} />
                      <YAxis tick={{ fontSize: 12 }} domain={[0, 1]} />
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="rejection_rate" name="Rejection rate" dot={false} />
                      <Line type="monotone" dataKey="retry_rate" name="Retry rate" dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div>
                <div className="text-xs font-medium text-zinc-700">Top issue codes (window)</div>
                        ...
                        {swipeIssues.length ? (
                          swipeIssues.map((r) => (
                            <tr key={r.code}>
                              ...
                              <Td className="text-right text-xs tabular-nums">{fmtInt((r as any).count)}</Td>
                            </tr>
                          ))
                        ) : (
                          <tr>
                            <Td colSpan={2} className="py-6 text-center text-xs text-zinc-600">
                              {qSwipeIngest.isLoading ? "Loading…" : "No issues recorded yet."}
                            </Td>
                          </tr>
                        )}
                      </tbody>
