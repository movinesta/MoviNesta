-- Session 20: Add served_dedupe_key breakdown columns to join integrity views.
--
-- We keep the existing column names for backward compatibility:
--   events_invalid_dedupe_key (counts NULL or invalid served_dedupe_key)
-- and add:
--   events_missing_served_dedupe_key
--   events_invalid_served_dedupe_key
...
WITH
  ...
  ev AS (
    SELECT
      e.event_day AS day,
      count(*) AS events,
      count(*) FILTER (WHERE e.served_dedupe_key IS NULL) AS events_missing_served_dedupe_key,
      count(*) FILTER (
        WHERE e.served_dedupe_key IS NOT NULL
          AND e.served_dedupe_key !~* '^[0-9a-f-]{36}:[0-9]+:[0-9a-f-]{36}$'
      ) AS events_invalid_served_dedupe_key,
...
SELECT
  ...
  COALESCE(ev.events_missing_served_dedupe_key, 0) AS events_missing_served_dedupe_key,
  COALESCE(ev.events_invalid_served_dedupe_key, 0) AS events_invalid_served_dedupe_key,
...
-- Preserve ownership + grants (re-apply defensively).
