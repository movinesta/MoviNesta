-- Session 18: Join integrity report for Swipe telemetry.
--
-- Purpose:
--   Detect disconnects between served impressions (rec_impressions) and outcomes/events (media_events)
--   by reporting daily counts, joinability rates, and invalid identifiers.
--
-- Implementation notes:
--   * Uses a security-invoker view so RLS policies of the invoking role apply (Postgres 15+).
--   * Joins events to impressions on (user_id, dedupe_key), which is the intended stable identity.
--   * Adds an index on rec_impressions(user_id, dedupe_key) to support joins.

BEGIN;

-- Support fast joins from media_events -> rec_impressions.
CREATE INDEX IF NOT EXISTS rec_impressions_user_dedupe_key_idx
  ON public.rec_impressions (user_id, dedupe_key);
...
-- Helper view: inspect a small sample of unjoinable events to debug payload/identity issues.
-- This view is intended for admins/service_role.
