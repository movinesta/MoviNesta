...
begin
  ...
  if to_regclass('public.assistant_reply_jobs') is not null then
  ...
  if to_regclass('public.ops_alerts') is not null then
    select coalesce(jsonb_agg(jsonb_build_object(
      'id', id,
      'fn', coalesce(nullif(meta->>'fn', ''), nullif(split_part(source, ':', 2), ''), source),
      'createdAt', created_at,
      'requestId', nullif(meta->>'request_id', ''),
      'conversationId', nullif(meta->>'conversation_id', ''),
      'userId', nullif(meta->>'user_id', ''),
      'code', coalesce(nullif(meta->>'code', ''), nullif(meta->'details'->'ai'->>'code', ''), 'UNKNOWN_ERROR'),
      'reason', coalesce(nullif(detail, ''), nullif(meta->'details'->'ai'->>'reason', ''), nullif(title, ''), 'Unknown error'),
      'culprit', meta->'details'->'ai'->'culprit',
      'context', coalesce(meta->'details'->'ai'->'context', meta->'details', '{}'::jsonb)
    ) order by created_at desc), '[]'::jsonb)
    into v_ai_failures
    from (
      select id, created_at, source, title, detail, meta
  ...
  if to_regclass('public.assistant_cron_requests') is not null then
    select coalesce(jsonb_agg(jsonb_build_object(
      'id', id,
      'job', job_name,
