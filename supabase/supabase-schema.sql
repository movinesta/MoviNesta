...
-- =====================================================================
-- Extra objects merged from test.sql (cleaned, runnable)
-- =====================================================================

-- Additional external ratings table
create table if not exists public.external_ratings (
  title_id text primary key references public.titles(id) on delete cascade,
  imdb_rating numeric,
  imdb_votes integer,
  rt_tomato_meter integer,
  rt_tomato_rating numeric,
  rt_tomato_user_meter integer,
  rt_tomato_user_rating numeric,
  metacritic_score integer,
  last_synced_at timestamptz not null default now()
);

-- RLS for external_ratings (read-only, dev-open)
...
create policy "external_ratings_select_all"
  on public.external_ratings
  for select
  using (true);

-- Extra TMDB-related columns on titles
alter table public.titles
...
-- NOTE: test.sql also contained a title_embeddings table where the
-- 'embedding' column has a USER-DEFINED type (likely a pgvector type).
-- Because the actual type name and dimension are unknown here, that block
-- is intentionally left commented out. Uncomment and adjust the type to
-- match your setup (e.g. vector(1536) or jsonb) before running.

-- create table if not exists public.title_embeddings (
--   title_id text primary key references public.titles(id) on delete cascade,
--   embedding vector(1536) not null,
--   source text default 'tmdb+omdb',
--   updated_at timestamptz not null default now()
-- );
