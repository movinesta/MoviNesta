...
export async function handler(req: Request) {
  ...
  let maxPerMinute = 60;
  try {
    const env = await loadAppSettingsForScopes(getAdminClient(req) as any, ["server_only"]);
    const cfg = (env.settings as any)["ops.rate_limits"];
    const v = (cfg && typeof cfg === "object" ? (cfg as any).actions?.["catalog-sync"] : null);
    if (typeof v === "number" && Number.isFinite(v)) {
      maxPerMinute = Math.max(1, Math.trunc(v));
    }
  } catch {
    // keep default
  }

  const rate = await enforceRateLimit(req, { action: "catalog-sync", maxPerMinute });
  ...
  const auth = await requireUserFromRequest(req, userClient, { mode: "user" });
