...
// 3.6) Supersede protection: if a newer user message exists, skip generating a reply for this older message.
    ...
    if (routed) {
      ...
      for (let loop = 0; loop < MAX_TOOL_LOOPS; loop++) {
        ...
        try {
          ...
          const preferredTimeout = Number((assistantSettings.params as any)?.timeout_ms ?? 12_000);
          const timeoutMs = llmTimeoutMs(preferredTimeout);
  ...
  if (status === 401 || status === 403) {
    finalReplyText =
      "I couldn’t reach the AI provider because the API key looks invalid or missing. Please verify OPENROUTER_API_KEY in Supabase secrets.";
  } else if (status === 400) {
    finalReplyText =
      "The AI provider rejected this request (400). This usually means an invalid model name or invalid request payload. Please check your assistant model settings in the Admin Dashboard.";
  } else if (status === 429) {
    finalReplyText =
      "The AI provider rate-limited this request. Please try again in a moment.";
  } else if (status === 503 || status === 504 || isTimeout) {
    finalReplyText =
      "I couldn’t reach the AI provider in time. Please try again in a moment.";
  } else if (msg === "Missing OPENROUTER_API_KEY") {
    finalReplyText =
      "OpenRouter is not configured (missing OPENROUTER_API_KEY). Add it in Supabase secrets, then redeploy the Edge Function.";
  } else {
    finalReplyText =
  ...
  break;
    ...
    const looksLikeMissingColumn = (err: any, col: string): boolean => {
      ...
      // Supabase/PostgREST error messages usually include: column "<col>" of relation "messages" does not exist
    ...
    if (insertRes.error) {
      ...
      const optionalCols = ["triggered_by_message_id", "client_id", "body"];
      ...
      // If meta/body is missing (very old schema), retry with the minimum viable row.
      if (insertRes.error && looksLikeMissingColumn(insertRes.error, "meta")) {
        insertAttempt = {
          conversation_id: conversationId,
          user_id: assistantUserId,
          sender_id: assistantUserId,
