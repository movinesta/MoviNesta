...
serve(async (req) => {
  ...
  try {
    ...
    const auth = await requireUserFromRequest(req, supabase);
    if (auth.errorResponse) return auth.errorResponse;
    const userId = auth.data!.userId;

    // Service-role client (best-effort) for reading server-only settings and building taste profiles.
    ...
    const maxDeckPerMin = clamp(Number((rateLimits as any).swipe_deck ?? 120) || 120, 1, 6000);

    // This endpoint is frequently hit during swipe sessions; throttle to protect DB/third-party rerank costs.
    const rl = await enforceRateLimit(req, { action: "swipe_deck", maxPerMinute: maxDeckPerMin });
    if (!rl.ok) {
      return json(req, 429, { ok: false, code: "RATE_LIMIT", retryAfterSeconds: rl.retryAfterSeconds ?? 5 });
    }
    ...
    if (requestedMode === "for_you" && !Boolean(body?.forceForYou)) {
      nStrong = await countStrongPosSignals(supabase, userId, { lookbackDays: coldStartLookbackDays, shortCircuitAt: coldStartMinStrong, eventLimit: coldStartEventLimit, thresholds: tasteThresholds });
    ...
    try {
      // Read negative outcomes from media_events (replaces rec_outcomes)
      // outcome_type mapping: dislike -> event_type='dislike'
      //                       not_interested -> event_type='dislike' with payload.action='not_interested'
      //                       hide -> event_type='dislike' with payload.action='hide'
      const { data: negOut } = await supabase
        .from("media_events")
        .select("media_item_id,event_type,payload,created_at")
        .eq("user_id", userId)
      ...
      for (const r of (negOut ?? []) as any[]) {
        ...
        // Derive outcome kind from payload.action
        const action = r?.payload?.action;
        const kind = action === "not_interested" ? "not_interested"
          : action === "hide" ? "hide"
            : "dislike";
    ...
    // from the user's preferred genres (user_preferences). This keeps personalization
    ...
    try {
      ...
      if (wantsSegFallback) {
        ...
        const recsys = (prefs?.recsys as any) || {};
        const preferred = Array.isArray(recsys.preferredGenres) ? recsys.preferredGenres : [];
        const muted = new Set<string>(
          (Array.isArray(recsys.mutedGenres) ? recsys.mutedGenres : [])
    ...
    try {
      ...
      if (applyMuted && userId) {
        ...
        const recsys = (prefs2?.recsys as any) || {};
        const muted2 = (Array.isArray(recsys.mutedGenres) ? recsys.mutedGenres : [])
          .map((g: any) => String(g ?? "").trim().toLowerCase())
          .filter(Boolean);
    ...
    if (rerankEnabled && !explicitQuery && canTasteRerank) {
      taste = await buildTasteQueryForUser(settingsClient, userId, tasteCfg as any, tasteMatchCfg);
    }
    ...
    if (canAttemptRerank) {
      ...
      const cooldownUntil = Math.max(await getCooldownUntil(cacheClient, userId), VOYAGE_RERANK_COOLDOWN_UNTIL);
      ...
      if (cached?.length) {
        ...
        try {
          ...
          if (msg.includes("(429)")) {
            ...
            await setCooldown(cacheClient, userId, rerankCooldown429Seconds, "voyage_429");
    ...
    const servedCardsRaw = (cards ?? []).slice(0, limit);

    // IMPORTANT: stamp a stable served position + dedupeKey on each card so client-side filtering
    // does not break joins between impressions/features/events.
    const servedCards = servedCardsRaw.map((c: any, idx: number) => ({
      ...c,
      position: idx,
      dedupeKey: `${recRequestId}:${idx}:${c.mediaItemId}`,
    }));
    ...
    // This should never block serving a deck.
    {
      ...
      try {
        const featureRows = servedCards.map((c, idx) => ({
          user_id: userId,
          dedupe_key: (c as any).dedupeKey ?? `${recRequestId}:${idx}:${c.mediaItemId}`,
        ...
        const { error: featErr } = await logClient
          ...
          .upsert(featureRows, { onConflict: "user_id,dedupe_key", ignoreDuplicates: true });
        void featErr;
      } catch {
        // ignore
      }
    ...
    try {
      ...
      // Store per-request context once.
      try {
        await logClient
          .from("rec_requests")
          .upsert(
      ...
      if (impressionRows.length) {
        const { error: impErr } = await logClient.from("rec_impressions").insert(impressionRows);
        void impErr;
      }
    } catch {
