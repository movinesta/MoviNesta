...
Deno.serve(async (req) => {
  ...
  // Suggestions can trigger tool execution; throttle to protect cost and abuse.
  const rl = await enforceRateLimit(req, { action: "assistant_suggestion_action", maxPerMinute: 60 });
  if (!rl.ok) {
