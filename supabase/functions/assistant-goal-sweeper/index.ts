...
serve(async (req) => {
  ...
  try {
    ...
    const { data: goals, error } = await svc
      ...
      .select("id, user_id, title, kind, status, end_at, meta")
      .eq("status", "active")
      .not("end_at", "is", null)
    ...
    for (const g of (goals ?? []) as GoalRow[]) {
      ...
      const s = (state as any as GoalStateRow) ?? null;
      const target = Number(s?.target_count ?? (g.meta as any)?.targetCount ?? 0);
      const progress = Number(s?.progress_count ?? 0);
