...
import { normalizeListName } from "./assistantToolArgs.ts";
import { TOOL_ARG_SCHEMAS, parseToolArgs } from "./assistantTools.argSchemas.ts";
import type { AssistantContentType, AssistantToolName, AssistantToolCall, AssistantToolSuccess, AssistantToolError, AssistantToolResult } from "./assistantTools.types.ts";
export type { AssistantContentType, AssistantToolName, AssistantToolCall, AssistantToolSuccess, AssistantToolError, AssistantToolResult } from "./assistantTools.types.ts";
...
// (moved to assistantTools.argSchemas.ts)
// -----------------------------------------------------------------------------
...
export async function toolGoalGetActive(supabase: any, userId: string) {
  try {
    const { data: goal, error } = await supabase
      .from("assistant_goals")
      .select("id, kind, title, status, end_at, meta, target_count, progress_count")
      .eq("user_id", userId)
      .eq("status", "active")
      .order("created_at", { ascending: false })
      .limit(1)
    ...
    return {
      ...
      targetCount: Number((goal as any).target_count ?? (goal as any)?.meta?.targetCount ?? 0),
      progressCount: Number((goal as any).progress_count ?? 0),
...
// --- OpenRouter native tool-calling support ---------------------------------
// Builds OpenRouter Responses API tool definitions from our tool registry.
// OpenRouter expects "OpenAI function calling format" with flat fields:
// { type:'function', name, description, strict, parameters }.
// See: https://openrouter.ai/docs/api/reference/responses/tool-calling
...
function toolNameToDescription(name: string): string {
  // Auto-generate a decent description without maintaining a giant manual map.
  // This is intentionally generic; the parameter schema + system prompt provide the specifics.
  const pretty = name.replace(/_/g, " ");
  const lead =
    name.startsWith("get_") ? "Get" :
      name.startsWith("search_") ? "Search" :
        name.startsWith("resolve_") ? "Resolve" :
          name.startsWith("list_") ? "List" :
            name.startsWith("review_") ? "Review" :
              name.startsWith("rating_") ? "Rating" :
                name.startsWith("message_") ? "Message" :
                  name.startsWith("goal_") ? "Goal" :
                    "Run";
  return `${lead} ${pretty}.`;
}

function isOptionalTypeName(typeName: string): boolean {
  return typeName === "ZodOptional" || typeName === "ZodDefault";
}

function unwrapZod(schema: z.ZodTypeAny): z.ZodTypeAny {
  let cur: any = schema;
  // Unwrap optional/default/effects layers until we reach a concrete schema.
  for (let i = 0; i < 8; i++) {
    const tn = cur?._def?.typeName;
    if (tn === "ZodOptional") cur = cur._def.innerType;
...
function zodToJsonSchemaLite(schema: z.ZodTypeAny): Record<string, unknown> {
  ...
  const tn: string = s?._def?.typeName ?? "ZodUnknown";

  // If optional/default were unwrapped, "required" is handled at object level.
  switch (tn) {
    case "ZodString":
      return { type: "string" };
    case "ZodNumber":
    ...
    case "ZodNativeEnum": {
      const vals = Object.values(s._def.values).filter((v) => typeof v === "string" || typeof v === "number");
      const t = vals.some((v) => typeof v === "number") ? "number" : "string";
      return { type: t, enum: vals };
    }
    case "ZodNullable": {
      const inner = zodToJsonSchemaLite(s._def.innerType);
      // best-effort: allow null via anyOf
      return { anyOf: [inner, { type: "null" }] };
    }
    case "ZodArray":
      return { type: "array", items: zodToJsonSchemaLite(s._def.type) };
    case "ZodTuple":
      return { type: "array", items: s._def.items.map((it: any) => zodToJsonSchemaLite(it)) };
    case "ZodUnion":
      return { anyOf: s._def.options.map((it: any) => zodToJsonSchemaLite(it)) };
    case "ZodRecord":
      return { type: "object", additionalProperties: zodToJsonSchemaLite(s._def.valueType) };
    case "ZodObject": {
      ...
      for (const [k, v] of Object.entries(shape ?? {})) {
        ...
        const vTypeName: string = vAny?._def?.typeName ?? "";
        // optional if it is ZodOptional/ZodDefault at top level
        if (!isOptionalTypeName(vTypeName)) required.push(k);
        properties[k] = zodToJsonSchemaLite(vAny as any);
      }
...
export function buildOpenRouterTools(
  names: AssistantToolName[],
  opts?: { descriptionOverrides?: Partial<Record<AssistantToolName, string>> },
): OpenRouterFunctionTool[] {
  const overrides = opts?.descriptionOverrides ?? {};
  return names.map((name) => {
    const schema = TOOL_ARG_SCHEMAS[name] ?? z.object({}).passthrough();
    const jsonSchema = zodToJsonSchemaLite(schema);
    // Ensure "parameters" is always a schema object
    const parameters =
      jsonSchema && typeof jsonSchema === "object" && Object.keys(jsonSchema).length
    ...
    return {
      type: "function",
      name,
      description: overrides[name] ?? toolNameToDescription(name),
      strict: null,
      parameters,
    };
  });
