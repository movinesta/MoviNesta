...
//
// Notes:
// - We default-deny unknown browser origins (no ACAO header).
// - Server-to-server requests (no Origin header) get '*' (no credentials).
// - You can override/extend allowed origins at deploy time via:
//     ALLOWED_ORIGINS="https://a.com,https://b.com,*.example.com"
...
function parseAllowedOriginsEnv(): OriginRule[] {
  ...
  for (const p of parts) {
    if (p.startsWith("*.") && p.length > 2) {
      rules.push({ kind: "wildcard", suffix: p.slice(1).toLowerCase() }); // includes leading '.'
      continue;
    }
    rules.push({ kind: "exact", value: p });
  }
  return rules;
}

const ENV_ORIGIN_RULES = parseAllowedOriginsEnv();

function isAllowedByEnv(origin: string): boolean {
  if (!ENV_ORIGIN_RULES.length) return false;
  for (const r of ENV_ORIGIN_RULES) {
    if (r.kind === "exact") {
      if (origin === r.value) return true;
      continue;
    }
    // wildcard by hostname suffix (requires https)
    try {
      const u = new URL(origin);
      if (u.protocol !== "https:") continue;
      const host = u.hostname.toLowerCase();
      if (host.endsWith(r.suffix)) return true;
    } catch {
      // ignore
    }
  }
...
function isMovinestaAppSubdomain(origin: string): boolean {
  try {
    const u = new URL(origin);
    if (u.protocol !== "https:") return false;
    const host = u.hostname.toLowerCase();
    return host === "movinesta.app" || host === "www.movinesta.app" || host.endsWith(".movinesta.app");
  } catch {
    return false;
  }
}
...
function pickAllowedOrigin(origin: string): string {
  if (!origin) return "";
  if (ORIGIN_ALLOWLIST.has(origin)) return origin;
  if (isMovinestaAppSubdomain(origin)) return origin;
  if (isAllowedByEnv(origin)) return origin;
  return "";
}
 ...
 *
 * Priority:
 * 1) x-request-id (from client/runner)
 * 2) cf-ray (Cloudflare)
 * 3) generated uuid
 */
...
export function corsHeadersFor(input?: Request | Headers | unknown): Record<string, string> {
  ...
  // Default-deny for unknown browser origins.
  // If there is no Origin header (server-to-server), allow '*' (no credentials).
  const allowOrigin = origin ? resolveAllowedOrigin(origin) : "*";

  const base: Record<string, string> = {
    "Access-Control-Allow-Headers":
      "authorization, x-client-info, apikey, content-type, x-job-token, x-request-id, x-runner-job-id",
    "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
    // Make these readable by the browser.
    "Access-Control-Expose-Headers": "x-request-id, x-runner-job-id, retry-after",
    "Vary": "Origin",
  };

  if (allowOrigin) {
    base["Access-Control-Allow-Origin"] = allowOrigin;
    // Only allow credentials for explicit origins (never for '*').
    if (allowOrigin !== "*") {
...
function resolveAllowedOrigin(origin: string): string {
  try {
    ...
    if (!o) return "";
    if (ORIGIN_ALLOWLIST.has(o)) return o;

    // Allow any HTTPS subdomain under movinesta.app (useful if you later host admin separately).
    try {
      const u = new URL(o);
      const host = u.hostname.toLowerCase();
      if (u.protocol === "https:" && (host === "movinesta.app" || host === "www.movinesta.app" || host.endsWith(".movinesta.app"))) {
        return o;
      }
    } catch {
      // ignore
...
export function jsonResponse(
  ...
  const baseHeaders: Record<string, string> = {
    ...
    "x-request-id": req ? getRequestId(req) : uuid(),
  };

  const headers = mergeHeaders(baseHeaders, init?.headers);

  // Always echo a correlation id.
  try {
    const rid = req ? getRequestId(req) : uuid();
    if (!headers.get("x-request-id")) headers.set("x-request-id", rid);
  } catch {
    // ignore
  }
...
export function jsonError(
  ...
  // If callers provide a retry-after hint (in seconds), also emit it as a header.
  // This helps browsers/clients implement backoff without parsing body.
  const retryAfterSeconds = (() => {
    try {
      const v = (extra as any)?.retryAfterSeconds ?? (extra as any)?.retry_after_seconds ?? (extra as any)?.retryAfter;
  ...
  // Include a correlation id in the error body for easy support/debugging.
  const rid = (() => {
    try {
      return req ? getRequestId(req) : uuid();
    } catch {
      return uuid();
    }
  })();
  (body as any).requestId = rid;
