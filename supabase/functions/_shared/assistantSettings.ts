  ...
  router: {
    ...
    policy: {
      ...
      provider: {
        order: string[];
        /**
         * Legacy routing fields (still supported for backward compatibility).
         * Prefer `only` + `ignore` going forward.
         */
        require: string[];
        allow: string[];
        ignore: string[];

        /** Preferred routing fields (OpenRouter-native) */
        only?: string[];
  ...
  router: {
    ...
    policy: {
      ...
      provider: {
        ...
        // sort is intentionally omitted by default to let OpenRouter choose the best provider.
      },
      variants: [],
    },
  },
  ...
  const behavior: AssistantBehavior = {
    ...
    router: {
      ...
      policy: (() => {
        ...
        const clampOptionalFloat = (v: unknown, min: number, max: number): number | undefined => {
          const n = typeof v === "number" ? v : typeof v === "string" ? Number(v) : NaN;
          if (!Number.isFinite(n)) return undefined;
          return Math.max(min, Math.min(max, n));
        };

        const clampOptionalPercentiles = (
          v: unknown,
          min: number,
          max: number,
        ): number | { p50?: number; p75?: number; p90?: number; p99?: number } | undefined => {
          const single = clampOptionalFloat(v, min, max);
          if (single !== undefined) return single;
          if (!isPlainObject(v)) return undefined;
          const out: any = {};
          for (const k of ["p50", "p75", "p90", "p99"] as const) {
            const n = clampOptionalFloat((v as any)[k], min, max);
            if (n !== undefined) out[k] = n;
          }
          return Object.keys(out).length ? out : undefined;
        };

        const providerSortVal = (provider as any)?.sort;
        let providerSort:
          | "price"
          | "throughput"
          | "latency"
        ...
        if (typeof providerSortVal === "string") {
          const s = providerSortVal.trim().toLowerCase();
          if (s === "price" || s === "throughput" || s === "latency") providerSort = s as any;
        } else if (isPlainObject(providerSortVal)) {
          const by = String((providerSortVal as any)?.by ?? "").trim().toLowerCase();
          if (by === "price" || by === "throughput" || by === "latency") {
            const p = String((providerSortVal as any)?.partition ?? "").trim().toLowerCase();
            const partition = p === "model" || p === "none" ? (p as "model" | "none") : undefined;
            providerSort = { by: by as any, ...(partition ? { partition } : {}) };
          }
        }

        const preferred_min_throughput = clampOptionalPercentiles(
          (provider as any).preferred_min_throughput,
          0,
          20000,
        );
        const preferred_max_latency = clampOptionalPercentiles((provider as any).preferred_max_latency, 0, 600);

        const maxPriceRaw = (provider as any).max_price;
        const max_price = (() => {
          if (!isPlainObject(maxPriceRaw)) return undefined;
          const out: any = {};
          for (const k of ["prompt", "completion", "request", "image"] as const) {
            const n = clampOptionalFloat((maxPriceRaw as any)[k], 0, 100000);
            if (n !== undefined) out[k] = n;
          }
          return Object.keys(out).length ? out : undefined;
        })();
        ...
        const enforce_distillable_text =
          typeof (provider as any).enforce_distillable_text === "boolean"
            ? ((provider as any).enforce_distillable_text as boolean)
            : undefined;
        return {
          mode,
          ...
          provider: {
            order: clampStringArray((provider as any).order, DEFAULT_BEHAVIOR.router.policy.provider.order, 12, 48),
            require: clampStringArray((provider as any).require, DEFAULT_BEHAVIOR.router.policy.provider.require, 12, 48),
            allow: clampStringArray((provider as any).allow, DEFAULT_BEHAVIOR.router.policy.provider.allow, 12, 48),
            ignore: clampStringArray((provider as any).ignore, DEFAULT_BEHAVIOR.router.policy.provider.ignore, 12, 48),
            only: clampStringArray((provider as any).only, (DEFAULT_BEHAVIOR.router.policy.provider as any).only ?? [], 12, 48),
            quantizations: clampStringArray((provider as any).quantizations, (DEFAULT_BEHAVIOR.router.policy.provider as any).quantizations ?? [], 12, 48),
            data_collection: (() => {
              const raw = String((provider as any).data_collection ?? "").trim().toLowerCase();
              return raw === "deny" ? "deny" : "allow";
            })(),
            zdr: clampBool((provider as any).zdr, false),
            require_parameters: (typeof (provider as any).require_parameters === "boolean") ? (provider as any).require_parameters : undefined,
            allow_fallbacks: clampBool(
              (provider as any).allow_fallbacks,
              DEFAULT_BEHAVIOR.router.policy.provider.allow_fallbacks,
            ),

            ...(providerSort ? { sort: providerSort } : {}),
            ...(preferred_min_throughput !== undefined ? { preferred_min_throughput } : {}),
            ...(preferred_max_latency !== undefined ? { preferred_max_latency } : {}),
            ...(max_price ? { max_price } : {}),
            ...(typeof enforce_distillable_text === "boolean" ? { enforce_distillable_text } : {}),
          },
