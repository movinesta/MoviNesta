import { normalizeOpenRouterBaseUrl, readOpenRouterCache, resolveOpenRouterBaseUrl, sanitizeOpenRouterBaseUrl } from "./openrouterCache.ts";
...
export async function loadOpenRouterZdrDiscovery(
  ...
  const row = await readOpenRouterCache(svc, "endpoints", baseUrl);
...
export async function resolveZdrRouting(opts: {
  ...
  if (candidate) {
    try {
      const safe = sanitizeOpenRouterBaseUrl(candidate);
      return {
        base_url: safe,
        discovery,
        meta: {
      ...
      // Invalid/untrusted base_url; fall back to the non-ZDR default.
    }
  }
