...
const ALLOWLIST = new Set<string>([
  ...
  // Any subdomain under movinesta.app (e.g., admin.movinesta.app)
  // Note: handled in pickOrigin() as a suffix allow, to keep the allowlist explicit.
  // Local dev
  "http://localhost:5173",
  "http://localhost:3000",
  "http://127.0.0.1:5173",
...
function pickOrigin(origin: string | null | undefined): string | null {
  ...
  // Allow any HTTPS subdomain under movinesta.app (useful for production admin dashboard hosting).
  try {
...
export function handleCorsPreflight(req: any): Response | null {
  ...
  const origin = pickOrigin(getHeader(req?.headers ?? req, "origin"));
  if (!origin) {
