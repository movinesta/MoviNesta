...
export async function safeCircuitGetSkipSet(
  ...
  try {
    ...
    // RPC: openrouter_circuit_should_skip_v1(p_models text[])
    const { data, error } = await svc.rpc("openrouter_circuit_should_skip_v1", { p_models: models });
...
export async function safeCircuitOnFailure(
  ...
  try {
    ...
    // RPC: openrouter_circuit_on_failure_v1(p_model text, p_status int, p_error text, p_retry_after_seconds int)
