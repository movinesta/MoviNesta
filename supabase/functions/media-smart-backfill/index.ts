...
async function getTmdbCache(kind: MediaKind, tmdbId: number): Promise<CacheEntry | null> {
  // TMDB IDs are not unique across media types, so we differentiate provider
  const provider = kind === "series" ? "tmdb:series" : "tmdb:movie";
  try {
    const { data, error } = await db
    ...
    return { raw: data.data, fetched_at: data.fetched_at };
  } catch {
    return null;
  }
}
...
async function putTmdbCache(kind: MediaKind, tmdbId: number, raw: any) {
  const provider = kind === "series" ? "tmdb:series" : "tmdb:movie";
  try {
    await db.from("media_metadata_cache").upsert({
      provider,
...
async function getOmdbCache(imdbId: string): Promise<CacheEntry | null> {
  try {
    const { data, error } = await db.from("media_metadata_cache")
      .select("data,fetched_at")
      .eq("provider", "omdb")
      .eq("external_id", imdbId)
      .maybeSingle();
    if (error || !data) return null;
    return { raw: data.data, fetched_at: data.fetched_at };
  } catch {
...
async function putOmdbCache(kind: MediaKind, imdbId: string, raw: any) {
  try {
    await db.from("media_metadata_cache").upsert({
      provider: "omdb",
      external_id: imdbId,
      media_type: kind, // optional but good to store
      fetched_at: nowIso(),
      data: raw
    });
  } catch {
