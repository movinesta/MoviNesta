...
import { nowIso, clampInt, toContextKey, ttlMinutesFor, cooldownMinutesFor, dailyCapFor } from "./lib/utils.ts";
...
async function loadAssistantPrefs(supabase: any, userId: string): Promise<AssistantPrefs> {
  try {
    ...
    // data.assistant is JSONB (camelCase)
    const assistant = (data?.assistant as any) || {};
    const enabled = typeof assistant.enabled === "boolean" ? assistant.enabled : true;
    const raw = assistant.proactivityLevel;
...
async function loadActiveGoalSummary(supabase: any, userId: string): Promise<ActiveGoalSummary | null> {
  try {
    ...
    const targetCountRaw = Number((goal as any)?.target_count ?? (goal as any)?.meta?.targetCount ?? (goal as any)?.meta?.target_count ?? 0);
    const progressCountRaw = Number((goal as any)?.progress_count ?? 0);
