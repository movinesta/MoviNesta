...
function toContextKey(surface: AssistantSurface, context: Record<string, unknown> | null | undefined) {
  ...
  if (surface === "search" && typeof c.query === "string") stable.query = String(c.query).slice(0, 60);
  // Home/diary: nothing else.
  return JSON.stringify(stable);
}

function ttlMinutesFor(surface: AssistantSurface) {
  switch (surface) {
    case "swipe":
    case "messages":
      return 30;
...
function cooldownMinutesFor(surface: AssistantSurface, proactivityLevel: 0 | 1 | 2): number {
  // Anti-annoyance: enforce a cooldown after a user dismisses a suggestion.
  // Higher proactivity -> shorter cooldown.
...
function dailyCapFor(proactivityLevel: 0 | 1 | 2): number {
...
async function loadAssistantStyle(supabase: any, userId: string): Promise<{
  ...
  try {
    ...
    if (error) {
      if (String(error.message ?? "").includes("assistant_memory")) {
        return { verbosityPreference: 0.45 };
      }
      return { verbosityPreference: 0.45 };
    }
    const value = (data as any)?.value ?? {};
    const v = Number(value?.verbosityPreference ?? 0.45);
    return { verbosityPreference: Number.isFinite(v) ? Math.max(0.1, Math.min(0.9, v)) : 0.45 };
  } catch {
    return { verbosityPreference: 0.45 };
  }
}
...
async function maybeRewriteCopy(
  ...
  const user = {
    suggestions: drafts.map((d) => ({ title: d.title, body: d.body })),
    avoid: (opts?.avoidPhrases ?? []).slice(0, 12),
  };
  ...
  try {
    ...
    const parsed = safeJsonParse<{ suggestions: { title: string; body: string }[] }>(res.content);
    if (!parsed?.suggestions?.length) return null;

    const rewritten = drafts.map((d, idx) => {
      const r = parsed.suggestions[idx];
      if (!r) return d;
      return {
        ...d,
        title: typeof r.title === "string" && r.title.trim() ? r.title.trim() : d.title,
        body: typeof r.body === "string" && r.body.trim() ? r.body.trim() : d.body,
      };
    });
...
async function maybePlanExtraDraftsWithPlanner(args: {
  ...
  const toolSummary = {
    create_list: { payload: { name: "string", description: "string?", isPublic: "boolean?", items: "{titleId,contentType,note?}[]?" } },
    diary_set_status: { payload: { titleId: "string", contentType: "movie|series|anime", status: "want_to_watch|watching|watched|dropped" } },
    list_add_item: { payload: { listId: "string", titleId: "string", contentType: "movie|series|anime", note: "string?" } },
    message_send: { payload: { conversationId: "string?", targetUserId: "string?", text: "string" } },
    playbook_start: { payload: { playbookId: "weekly_watch_plan" } },
    playbook_end: { payload: { playbookId: "weekly_watch_plan?" } },
    navigate: { payload: { to: "string" } },
    toolchain: { payload: { steps: "{tool,args}[]", navigateStrategy: "first|last|none?" } },
    dismiss: {},
  };

  const sys =
    "You are an assistant PLANNER embedded inside a movie discovery app. " +
    "Your job is to propose proactive, context-aware suggestions that trigger concrete tools. " +
  ...
  const user = {
    ...
    activeGoal: goalText,
    needCount: need,
    avoidTitles: args.avoidTitles.slice(0, 8),
    allowedActionTypes: Object.keys(toolSummary),
    toolSchemas: toolSummary,
    outputSchema: {
      suggestions: [
        {
          kind: "string",
          title: "string (<= 7 words)",
          body: "string (<= 28 words)",
          actions: "AssistantAction[]",
    ...
    rules: [
      "Be proactive and specific; no generic 'ask me anything'.",
      "No emojis.",
      "Do not mention being an AI or a model.",
      "Prefer actions that reduce friction (one tap).",
      "Always include a Dismiss action.",
    ],
  };
  ...
  try {
    ...
    const parsed = safeJsonParse<PlannerResponse>(res.content);
    const suggestions = (parsed?.suggestions ?? []).slice(0, need);
    ...
    const drafts: DraftSuggestion[] = suggestions
      .map((s, idx) => {
        ...
        const kind = typeof s.kind === "string" && s.kind.trim() ? s.kind.trim() : "insight";
        if (!title || !body) return null;
        const actions = sanitizeDraftActions(s.actions);
        const score = typeof s.score === "number" && Number.isFinite(s.score) ? Math.max(0, Math.min(1, s.score)) : 0.25;
...
async function maybeCriticRankDrafts(args: {
  ...
  const sys =
    ...
    "You must be strict about relevance, redundancy, and annoyance. " +
    "Return ONLY valid JSON.";

  const user = {
    surface: args.surface,
    avoidTitles: args.avoidTitles.slice(0, 8),
    items: args.drafts.map((d) => ({ kind: d.kind, title: d.title, body: d.body })),
    outputSchema: { items: [{ keep: "boolean", score: "number 0..1" }] },
    rules: [
      "Drop items that are repetitive or generic.",
      "Score higher if it's actionable and perfectly matched to the surface.",
      "Prefer shorter, sharper copy.",
      "No more than one item about the same exact next step.",
    ],
  };
  ...
  try {
    ...
    const parsed = safeJsonParse<CriticResponse>(res.content);
    ...
    const scored = args.drafts
      .map((d, idx) => {
        const it = items[idx] ?? {};
        const keep = typeof it.keep === "boolean" ? it.keep : true;
        const s = typeof it.score === "number" && Number.isFinite(it.score) ? Math.max(0, Math.min(1, it.score)) : d.score ?? 0.25;
        return { d, keep, s };
      })
      .filter((x) => x.keep)
      .sort((a, b) => b.s - a.s)
...
Deno.serve(async (req) => {
  ...
  const validated = await validateRequest(
    ...
    (body) => {
      ...
      const limit = clampInt(Number(b?.limit ?? 3), 1, 6);
      return { surface, context, limit };
    },
    { logPrefix: "[assistant-orchestrator]", requireJson: true },
  );
  if (validated.errorResponse) return validated.errorResponse;
  const { surface, context, limit } = validated.data;
  ...
  const ttlMin = ttlMinutesFor(surface);
  ...
  const style = await loadAssistantStyle(supabase, userId);

  const cooldownMin = cooldownMinutesFor(surface, prefs.proactivityLevel);
  const inCooldown = await isInDismissCooldown(supabase, userId, surface, cooldownMin);
  if (inCooldown) {
    return jsonResponse(req, { ok: true, contextKey, activeGoal: activeGoalSummary, suggestions: [] });
  }

  const cap = dailyCapFor(prefs.proactivityLevel);
