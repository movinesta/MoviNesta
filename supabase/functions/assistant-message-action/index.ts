...
function parseRequestBody(raw: unknown): RequestBody {
  ...
  return {
    ...
    confirmToken: typeof o.confirmToken === "string" ? o.confirmToken : undefined,
  };
}
...
export async function handler(req: Request) {
  ...
  try {
    ...
    const rl = await enforceRateLimit(req, { action: "assistant_message_action", maxPerMinute: 60 });
    if (!rl.ok) {
      return jsonError("Rate limit exceeded", 429, "RATE_LIMIT", req, { retryAfterSeconds: rl.retryAfterSeconds });
    }

    const { data: body, errorResponse } = await validateRequest<RequestBody>(
      req,
      parseRequestBody,
      { requireJson: true, logPrefix: `${FN_NAME}(${requestId})` },
    );
