...
// Controlled via admin App Settings (server_only) where possible.
...
async function maybeRerank(
  ...
  // Admin-controlled server-only settings (fallback to safe defaults if unavailable).
  let rerankModel = "rerank-2.5";
  let cacheTtlSec = DEFAULT_CACHE_TTL_SECONDS;
  let cooldown429Sec = DEFAULT_429_COOLDOWN_SECONDS;

  try {
    const defaults = getDefaultSettingsForScope("server_only");
    const { settings: serverOnly } = await loadAppSettingsForScopes(adminClient, ["server_only"], { cacheTtlMs: 60_000 });
    const merged: Record<string, any> = { ...defaults, ...serverOnly };
    const rawModel = String(merged["ml.rerank.model"] ?? "rerank-2.5").trim();
    if (rawModel) rerankModel = rawModel;

    const ttl = Number(merged["ops.rerank.cache_ttl_seconds"] ?? DEFAULT_CACHE_TTL_SECONDS);
    if (Number.isFinite(ttl)) cacheTtlSec = Math.max(30, Math.min(Math.floor(ttl), 86_400));

    const cd = Number(merged["ops.rerank.cooldown_429_seconds"] ?? DEFAULT_429_COOLDOWN_SECONDS);
    if (Number.isFinite(cd)) cooldown429Sec = Math.max(10, Math.min(Math.floor(cd), 3_600));
  } catch {
  ...
  const { results } = await voyageRerank(query, docs, {
    ...
    console.log(
      ...
      JSON.stringify({ model: rerankModel, queryLen: query.length, in: rows.length, rerankCandidates: k, scored: order.length }),
    );
    ...
    if (is429(err) && userId) {
      const cdKey = `cooldown:voyage_search_rerank:${userId}`;
      const until = addSeconds(new Date(), cooldown429Sec);
      await cacheUpsert(adminClient, {
        key: cdKey,
        user_id: userId,
        order_ids: "[]",
...
async function handleSearch(
  ...
  builder = builder.textSearch("search_vector", queryClean, {
    config: "english",
    type: "websearch",
  });
...
export async function handler(req: Request) {
  ...
  const supabaseServiceRoleKey = cfg.supabaseServiceRoleKey ?? "";
  ...
  const apiKeyProvided = (req.headers.get("apikey") ?? req.headers.get("x-api-key") ?? "").trim();
  const apiKeyHeader = apiKeyProvided || supabaseAnonKey;

  // Fail fast for misconfigured clients: require a valid Supabase key header.
  // (Edge runtime does not always implicitly validate apikey during key transitions.)
  if (!apiKeyHeader || (apiKeyHeader !== supabaseAnonKey && apiKeyHeader !== supabaseServiceRoleKey)) {
  ...
  // Require an authenticated session for search (prevents anonymous scraping & cost spikes).
  // Verify caller (verify_jwt may be disabled for JWT Signing Keys).
  ...
  if (!userId) {
    ...
    if (userErr || !userId) return jsonError(req, "Unauthorized", 401, "UNAUTHORIZED");
  }

  // Rate limit per user / IP (DB-backed when available).
  const rl = await enforceRateLimit(req, { action: "media_search", maxPerMinute: 120 });
  if (!rl.ok) {
