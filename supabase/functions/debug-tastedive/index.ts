...
serve(async (req: Request) => {
  ...
  if (!apiKey) {
    return jsonError("Missing secret: TASTEDIVE_API_KEY", 500, "MISSING_SECRET");
  }
  ...
  if (!q) return jsonError("Body must include: { q: string }", 400, "BAD_REQUEST");
  if (type !== "movie" && type !== "show") {
    return jsonError(`type must be "movie" or "show"`, 400, "BAD_REQUEST");
  }
  ...
  try {
    ...
    return jsonResponse(
      { ok: false, stage: "fetch", error: String(e?.message ?? e) },
      502,
    );
  }
