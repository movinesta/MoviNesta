...
function labelForEvent(eventType: string, payload: Json, rating0_10?: number | null, inWatchlist?: boolean | null): string | null {
  if (eventType === "like") {
    const action = payload && typeof payload === "object" ? (payload as any).action : undefined;
    if (action === "status") return "status_watched";
    return "swipe_like";
  }
  if (eventType === "dislike") return "swipe_dislike";
...
serve(async (req) => {
  ...
  try {
    ...
    let body: BaseBody;
    try {
      body = (await req.json()) as BaseBody;
    } catch {
    ...
    const incomingEvents: BatchEvent[] = Array.isArray(body.events) && body.events.length
      ? body.events
      : [
          {
...
const runBackgroundTasks = async () => {
  ...
  for (const op of ratingOps) {
    try {
      ...
      if (op.rating0_10 === null) {
        await supabase.from("ratings").delete().eq("user_id", user.id).eq("title_id", op.mediaItemId);
      } else {
        await supabase
          .from("ratings")
          .upsert(
  ...
  if (hasStrongPositive) {
    ...
    if (shouldRefresh) {
      await supabase.rpc("media_refresh_user_centroids_v1", { p_user_id: user.id }).catch(() => null);
    }
  }
...
const bg = runBackgroundTasks().catch(() => null);
