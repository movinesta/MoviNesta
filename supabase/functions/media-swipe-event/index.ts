...
serve(async (req) => {
  ...
  try {
    ...
    // Service-role client (if available) for reading server-only app settings.
    const SERVICE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "";
    const svc = SERVICE_KEY
      ? createClient(supabaseUrl, SERVICE_KEY, { auth: { persistSession: false } })
      : null;
    ...
    if (!rl.ok) {
      return new Response(JSON.stringify({ ok: false, code: "RATE_LIMIT", retryAfterSeconds: rl.retryAfterSeconds ?? 5 }), {
    ...
    const syntheticWatchedDwellMs = clampNumber((serverOnly as any)["ranking.swipe.status_watched.synthetic_dwell_ms"] ?? 12000, 0, 600_000);
    const centroidRefreshSampleRate = clampNumber((serverOnly as any)["ranking.swipe.centroids.refresh_sample_rate"] ?? 0.25, 0, 1);
    const centroidK = clampNumber((serverOnly as any)["ranking.swipe.centroids.k"] ?? 3, 1, 10);
    const centroidMaxItems = clampNumber((serverOnly as any)["ranking.swipe.centroids.max_items"] ?? 60, 10, 200);
    ...
    const ltrLabelsByKey = new Map<string, Set<string>>();
    ...
    for (const body of items) {
      ...
      for (const ev of incomingEvents) {
        ...
        if (eventType === "like" && payload && typeof payload === "object" && (payload as any).action === "status") {
          ...
          // IMPORTANT: diary "watched" should influence taste updates & feedback.
          // We do NOT log it as a "like" (that would be too strong). Instead, emit a synthetic dwell.
        ...
        if (ltr && deckId) {
          const k = `${deckId}|${mediaItemId}`;
          let set = ltrLabelsByKey.get(k);
          if (!set) {
            set = new Set<string>();
            ltrLabelsByKey.set(k, set);
          }
          set.add(ltr);
        }
    ...
    // DEPRECATED: rec_outcomes table merged into media_events. Keeping this block
    // commented for reference during migration period.
    try {
      if (rows.length) {
        const impressionRows = rows
          .filter((r: any) => String(r.event_type) === "impression")
          .map((r: any) => ({
            user_id: r.user_id,
        ...
        if (impressionRows.length) {
          ...
          if (impErr) console.warn("Impression log failed", impErr);
        }
      }

      // rec_outcomes write REMOVED - consolidated into media_events
      // All outcome data is now derived from media_events with rec_request_id populated
    } catch {
      // ignore
    }
    ...
    const runBackgroundTasks = async () => {
      ...
      // LTR training labels (best-effort): update feature log labels for every (deck,item) we touched.
      if (ltrLabelsByKey.size) {
        for (const [k, labels] of ltrLabelsByKey.entries()) {
          const parts = k.split("|");
          const dId = parts[0] ?? "";
          const mId = parts[1] ?? "";
          if (!dId || !mId || !labels?.size) continue;
          ...
          try {
            ...
            if (existing?.id) {
              ...
              for (const l of Array.from(labels)) pieces.add(l);
    ...
    const needsBg = (() => {
      ...
      // Taste updates only for non-impressions.
      const hasTasteRelevant = rows.some((r) => {
        const et = String((r as any)?.event_type ?? (r as any)?.eventType ?? "");
        return [
          "like",
          "dislike",
          "skip",
