...
export async function handler(req: Request) {
  ...
  try {
    ...
    if (auth.errorResponse) {
      log(logCtx, "Auth error", { error: "unauthorized" });
    ...
    if (req.method === "GET") {
      return await handleGet(req, supabase, userId);
    }
    if (req.method === "POST") {
      return await handlePost(req, supabase, userId);
    }
...
async function handleGet(req: Request, supabase: SupabaseClient<Database>, userId: string): Promise<Response> {
  ...
  if (error) {
    log({ fn: FN_NAME }, "Failed to load preferences", { userId, error: error.message });
...
async function handlePost(req: Request, supabase: SupabaseClient<Database>, userId: string): Promise<Response> {
  ...
  if (error || !data) {
    log({ fn: FN_NAME }, "Failed to save preferences", { userId, error: error?.message });
    return jsonError(req, "Failed to save preferences", 500, "PREFERENCES_SAVE_FAILED");
  }
