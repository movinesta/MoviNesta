...
function splitTopLevelCommaList(raw) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    // Handle single-quoted strings (defaults might contain strings)
    if (ch === "'") {
...
function extractPublicFunctions(rawSql) {
  // Parse function signature lines. pg_dump prints a single-line header like:
  //   CREATE FUNCTION public.foo(p_id uuid, p_limit integer DEFAULT 10) RETURNS SETOF public.bar
  // Followed by LANGUAGE/AS on subsequent lines.
  const funcs = new Map();

  for (const line of rawSql.split(/\r?\n/)) {
    if (!/^CREATE FUNCTION\s+public\./i.test(line)) continue;

    const m = line.match(
      /^CREATE FUNCTION\s+public\.([a-zA-Z0-9_]+)\s*\((.*)\)\s+RETURNS\s+(.+)$/i,
    );
    if (!m) continue;

    const [, fnName, argsRaw, returnsRaw] = m;
    funcs.set(fnName, {
      name: fnName,
      argsRaw: String(argsRaw ?? ""),
      returnsRaw: String(returnsRaw ?? "").trim(),
    });
  }
...
function normalizePgType(typeText) {
  return normalizeTypeName(String(typeText ?? "")).trim();
}

function mapReturnTypeToTs(returnsRaw, enumValues, tableNames) {
  let r = String(returnsRaw ?? "").trim().replace(/\s{2,}/g, " ");
  if (!r) return "unknown";

  const stripPublic = (x) => x.replace(/^public\./i, "");

  if (/^void\b/i.test(r)) return "undefined";
  if (/^trigger\b/i.test(r)) return "unknown";

  // RETURNS TABLE(...) => treat as Json[] (row objects)
  if (/^TABLE\s*\(/i.test(r)) return "Json[]";

  let isSetof = false;
  if (/^SETOF\s+/i.test(r)) {
    isSetof = true;
    r = r.replace(/^SETOF\s+/i, "").trim();
  }

  const base = stripPublic(normalizePgType(r));
  let ts;
  if (tableNames.includes(base)) {
    ts = `Database["public"]["Tables"]["${base}"]["Row"]`;
  } else {
    ts = mapTypeTextToTs(base, enumValues);
  }

  return isSetof ? `${ts}[]` : ts;
}

function parseFunctionArgsToTs(argsRaw, enumValues) {
  const parts = splitTopLevelCommaList(argsRaw);
  if (!parts.length) return { argsLines: null };
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    p = p.replace(/^(INOUT|IN|OUT|VARIADIC)\s+/i, "");

    const hasDefault = /\bDEFAULT\b/i.test(p) || /\s=\s/.test(p);

    p = p.replace(/\bDEFAULT\b[\s\S]*$/i, "").trim();
    p = p.replace(/\s=\s[\s\S]*$/i, "").trim();
    ...
    const tsType = mapTypeTextToTs(typeText, enumValues);
    const finalTs = `${tsType} | null`;

    argLines.push(`          ${formatKey(name)}${hasDefault ? "?:" : ":"} ${finalTs};`);
  }

  return { argsLines: argLines };
}
...
(function main() {
  ...
  if (functionDefs.size === 0) {
    ...
    for (const [fnName, def] of functionDefs.entries()) {
      const args = parseFunctionArgsToTs(def.argsRaw, enumValues);
      const returnsTs = mapReturnTypeToTs(def.returnsRaw, enumValues, tableNames);
      lines.push(`      ${formatKey(fnName)}: {`);
      if (!args.argsLines) {
        lines.push("        Args: Record<string, never>;");
      } else {
        lines.push("        Args: {");
        for (const l of args.argsLines) lines.push(l);
        lines.push("        };");
      }
      lines.push(`        Returns: ${returnsTs};`);
      lines.push("      };");
    }
  }
  lines.push("    };");
