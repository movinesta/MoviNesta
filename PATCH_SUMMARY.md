...
## Messages (Round 8)
- Added **timed mute** presets (1h / 8h / 24h) + **indefinite mute**.
- Added `conversation_prefs.muted_until` (timestamptz) and updated `get_conversation_summaries_v2` to return `self_muted_until`.
- `self_muted` now evaluates true if `muted=true` **or** `muted_until > now()`.
- Conversation info sheet now shows **“Muted until …”** when using timed mutes.
- Backward compatible: if the backend column isn’t migrated yet, timed mutes still work locally.
