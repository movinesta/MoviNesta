...
const NotificationsForm: React.FC<{
  ...
  user: { id: string };
  onSave: (prefs: NotificationPrefs) => Promise<NotificationPrefs & { updatedAt: string }>;
}> = ({ initialPrefs, user, onSave }) => {
    ...
    <section className="space-y-[var(--section-gap)] page-pad pb-2">
      ...
      <div className="space-y-3 rounded-2xl border border-border/60 bg-card/80 p-[var(--card-pad)] shadow-sm">
        ...
        <div className="space-y-2 pt-2 text-[12px]">
          <label className="soft-row-card soft-row-card-interactive row-pad flex items-start justify-between gap-3">
            <div>
            ...
            <input
              ...
              className="mt-0.5 h-5 w-5 rounded border-border bg-background text-primary"
...
const SettingsNotificationsPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 pb-2 pt-1">
      ...
      <div className="mx-4 flex items-start gap-2 rounded-md border border-border bg-card/70 p-3 text-[12px] text-muted-foreground shadow-sm">
