...
function CircleIconButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="flex size-11 items-center justify-center rounded-full border border-border/60 bg-muted/60 text-foreground backdrop-blur-md transition hover:bg-muted active:scale-[0.98]"
    >
      {children}
    </button>
  );
...
function ReviewCard({
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/70 card-pad">
        ...
        <div className="text-sm leading-relaxed text-muted-foreground">
          {canShow ? (
            ...
            <div className="rounded-xl border border-border bg-muted/40 p-3">
              <p className="text-muted-foreground">Spoiler warning</p>
              <button
                type="button"
      ...
      <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground">
        ...
        <button
          type="button"
          className="ml-auto rounded-full bg-muted/60 px-3 py-1 text-xs font-semibold text-foreground hover:bg-muted"
        >
          Reply
        </button>
      </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg border-border bg-background text-foreground">
        <DialogHeader>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{titleName}</p>
        </DialogHeader>
        ...
        <div className="space-y-5">
          <div>
            <div className="flex items-center justify-between">
              ...
              <p className="text-sm font-semibold text-foreground">{ratingStars.toFixed(1)}/5</p>
            </div>
            <div className="mt-2">
          ...
          <div>
            ...
            <Textarea
              ...
              className="min-h-[120px] border-border bg-background text-foreground placeholder:text-muted-foreground"
            />
          </div>

          <label className="flex items-center justify-between rounded-xl border border-border bg-muted/40 px-3 py-2.5">
          ...
          <div className="flex gap-3">
            <Button
              variant="secondary"
              className="flex-1 border border-border bg-muted/60 text-foreground hover:bg-muted"
...
export default function TitleReviewsPageV2() {
  ...
  if (!canUseId) {
    return (
      <div className="px-[var(--page-pad-x)] py-10 text-center text-muted-foreground">
  ...
  return (
    <div className="relative min-h-[calc(100dvh-(5.5rem+env(safe-area-inset-bottom)))] bg-background pb-24">
      {/* Top bar */}
      <div className="sticky top-0 z-30 flex items-center justify-between border-b border-border/60 bg-background/80 px-[var(--page-pad-x)] pb-3 pt-[calc(env(safe-area-inset-top)+0.75rem)] backdrop-blur relative full-bleed">
      ...
      <div className="px-[var(--page-pad-x)] pt-6">
        ...
        <div className="flex flex-col stack-gap">
          ...
          {reviewsQuery.hasNextPage ? (
            <Button
              variant="secondary"
              className="w-full rounded-full border border-border bg-muted/60 text-foreground hover:bg-muted"
      ...
      {/* Floating action */}
      <div className="fixed inset-x-0 bottom-[calc(5.5rem+env(safe-area-inset-bottom))] z-40 px-[var(--page-pad-x)]">
