...
const ConversationListRow: React.FC<{
  ...
  dimmed?: boolean;
}> = ({ conversation, isMuted, onOpenActions, onOpenConversation, dimmed = false }) => {
    ...
    <li>
      <Link
        ...
        onClick={(e) => {
          ...
          // Don't run side effects for modified clicks (new tab/window, context actions, etc.)
...
const trimmedQuery = query.trim().toLowerCase();
  ...
  const toggleMute = (conversation: ConversationListItem) => {
    ...
    if (conversation.isMuted) {
      ...
      saveConversationPrefs(userId, conversation.id, {
        ...
        .catch((err) => {
          console.warn("[MessagesPage] Failed to save conversation prefs (unmute)", err);
          updateConversationListItemInCache(queryClient, userId, conversation.id, (c) => ({
            ...c,
            isMuted: true,
            mutedUntil: prevUntil,
  ...
  const applyMutePreset = (conversation: ConversationListItem, preset: MutePreset) => {
    ...
    saveConversationPrefs(userId, conversation.id, {
      ...
      .catch((err) => {
        console.warn("[MessagesPage] Failed to save conversation prefs (mute preset)", err);
        updateConversationListItemInCache(queryClient, userId, conversation.id, (c) => ({
          ...c,
          isMuted: prevMuted,
          mutedUntil: prevUntil,
  ...
  const toggleHidden = (conversation: ConversationListItem) => {
    ...
    saveConversationPrefs(userId, conversation.id, {
      ...
      .catch((err) => {
        console.warn("[MessagesPage] Failed to save conversation prefs (toggle hidden)", err);
        updateConversationListItemInCache(queryClient, userId, conversation.id, (c) => ({
          ...c,
          isHidden: !nextHidden,
        }));
  ...
  const markAsRead = async (conversation: ConversationListItem) => {
    ...
    try {
      ...
      console.warn("[MessagesPage] Failed to mark conversation read", err);
      toast.show("Couldn't mark as read. Please try again.");
    }
  };
        ...
        <div className="full-bleed flex space-x-4 overflow-x-auto border-b border-border/60 pb-3 pl-[var(--page-pad-x)] pr-[var(--page-pad-x)] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {storyParticipants.map((participant) => (
            <button
              ...
              className="flex shrink-0 flex-col items-center gap-1"
              onClick={() => {
                const conv = (data ?? []).find((c) => c.id === participant.conversationId);
                if (conv?.hasUnread) void markAsRead(conv);
                navigate(`/messages/${participant.conversationId}`);
              }}
            >
              <div className="relative h-14 w-14 overflow-hidden rounded-full border-2 border-primary/50">
                {participant.avatarUrl ? (
