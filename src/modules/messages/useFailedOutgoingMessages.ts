...
export function useFailedOutgoingMessages(args: {
  ...
  // Reset state whenever we switch conversations.
  useEffect(() => {
    setSendError(null);
  ...
  useEffect(() => {
    return () => {
      ...
      void supabase.storage
        ...
        .then(({ error }) => {
          if (error) {
            console.error("[useFailedOutgoingMessages] Failed to cleanup orphaned attachments", error);
          }
        })
        .catch((err) => {
  ...
  const onSendFailed = useCallback(
    (tempId: string, payload: FailedMessagePayload) => {
