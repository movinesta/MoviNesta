...
const ConversationPage: React.FC = () => {
  ...
  const unmuteConversation = useCallback(() => {
    ...
    saveConversationPrefs(currentUserId, conversationId, {
      ...
      .catch((err) => {
        console.warn("[ConversationPage] Failed to save conversation prefs (unmute)", err);
        updateConversationListItemInCache(queryClient, currentUserId, conversationId, (c) => ({
          ...c,
          isMuted: prevMuted,
          mutedUntil: prevUntil,
  ...
  const applyMutePreset = useCallback(
    (preset: MutePreset) => {
      ...
      saveConversationPrefs(currentUserId, conversationId, {
        ...
        .catch((err) => {
          console.warn("[ConversationPage] Failed to save conversation prefs (mute preset)", err);
          updateConversationListItemInCache(queryClient, currentUserId, conversationId, (c) => ({
            ...c,
            isMuted: prevMuted,
            mutedUntil: prevUntil,
    ...
    async (userMessageId: string) => {
      ...
      try {
        ...
        const requestAssistant = async (stream: boolean) => {
          ...
          const res = await fetchSupabaseEdgeFunction("assistant-chat-reply", {
            method: "POST",
            accept: stream ? "text/event-stream" : "application/json",
            headers,
            body: { conversationId, userMessageId, stream },
    ...
    async (messageId: string, actionId: string) => {
      ...
      try {
        const data = await callSupabaseFunction<any>(
          ...
          { conversationId, messageId, actionId },
          { requireAuth: true },
        );
  ...
  if (!conversationId) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center page-pad">
  ...
  return (
    <div className="conversation-page relative flex h-[100dvh] w-full flex-col items-stretch overflow-hidden bg-background text-foreground">
      <div className="mx-auto flex h-full w-full max-w-3xl flex-1 min-h-0 flex-col items-stretch rounded-none border border-border bg-background">
        ...
        >
          <header className="flex items-center justify-between page-pad py-3">
              ...
              <div className="flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="block w-full rounded-full border border-input bg-background py-2.5 pl-10 pr-10 text-sm text-foreground placeholder:text-muted-foreground shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                    autoComplete="off"
                    spellCheck={false}
                  />
                  {search.query.trim() && (
                    <button
                      type="button"
                      className="absolute inset-y-0 right-0 icon-hit"
        ...
        <section className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-muted/10">
            {pollWhenRealtimeDown && (
              <div className="pointer-events-none absolute inset-x-0 top-2 z-20 flex justify-center page-pad">
                <div className="pointer-events-auto inline-flex items-center gap-2 rounded-full border border-border/60 bg-background/85 px-3 py-1 text-[12px] text-muted-foreground shadow-sm backdrop-blur">
            ...
            <MessageList
              ...
              errorContent={
                isMessagesError ? (
                  <div className="mb-3 rounded-2xl border border-destructive/25 bg-destructive/10 px-3 py-2 text-[12px] text-destructive">
          ...
          {blockedYou && (
            <div className="sticky bottom-0 z-10 flex-shrink-0 border-t border-border bg-background/95 page-pad py-3 text-center text-xs text-muted-foreground">
