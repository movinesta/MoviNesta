...
const ConversationPage: React.FC = () => {
  ...
  const isAssistantThread = useMemo(() => {
    ...
    if (!p) return false;
    return p.username?.toLowerCase() === ASSISTANT_USERNAME;
  }, [assistantCache?.conversationId, conversationId, otherParticipant]);
  ...
  const triggerAssistantReply = useCallback(
    async (userMessageId: string) => {
      ...
      try {
        ...
        if (error) {
          setAssistantReplyFailed({ userMessageId, error: error.message || "Assistant failed" });
        } else if (!data?.ok) {
          setAssistantReplyFailed({
            userMessageId,
            error: (data as any)?.code ? String((data as any).code) : "Assistant failed",
          });
        }
      } catch (e: any) {
        ...
        },
        {
          onSuccess: async (sentMessage) => {
            ...
            if (
              ...
              triggerAssistantReply(sentMessage.id);
            }
          },
        },
      );
