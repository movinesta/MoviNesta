...
export const createSignedStorageUrl = async (
  ...
  setCachedSignedUrl(`${bucket}:${path}`, data.signedUrl, ttlSeconds);
...
export async function resolveStorageUrl(
  ...
  if (!path) return null;
  if (isHttpUrl(path)) {
    if (!opts?.allowHttp) return null;
    return path;
  }

  const ttlSeconds = opts?.ttlSeconds ?? DEFAULT_SIGNED_URL_TTL_SECONDS;
  const allowPublicFallback = opts?.allowPublicFallback ?? false;
...
export async function prefetchSignedStorageUrl(
  ...
  if (getCachedSignedUrl(`${bucket}:${path}`)) return;
