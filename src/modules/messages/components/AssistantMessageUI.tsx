...
export type AssistantUiPayload = {
  ...
  citations?: AssistantUrlCitation[] | null;
};

export type AssistantUrlCitation = {
  url: string;
  title?: string;
  domain?: string;
};
...
export default function AssistantMessageUI({
  ...
  const citations = useMemo(() => {
    ...
    for (const c of raw) {
      ...
      out.push({
        ...
        domain: typeof c?.domain === "string" && c.domain.trim() ? c.domain.trim() : undefined,
      });
    ...
    return (
      <div className="rounded-2xl border border-border/60 bg-card/40 p-3">
        ...
        >
          ...
          <span className="text-xs text-muted-foreground">{sourcesOpen ? "Hide" : "Show"}</span>
        </button>
  ...
  if (!payload || (payload.layout ?? "stacked") !== "stacked") {
    ...
    if (ids.length === 0 && !citations.length) return null;
    return (
      <div className="mt-2 space-y-2">
        {renderSources()}
        {ids.length ? <div>{renderActionButtons(ids)}</div> : null}
      </div>
    );
  }
