...
export default function AssistantMessageUI({
  ...
  if ((payload.layout ?? "stacked") !== "stacked") return null;

  const cards = Array.isArray(payload.cards) ? payload.cards : [];
  if (!payload.heading && cards.length === 0) return null;
              ...
              <div className="min-w-0 flex-1">
                ...
                {actionIds.length ? (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {actionIds.slice(0, 4).map((id) => {
                      const action = actionsById.get(id);
                      if (!action) return null;
                      const label = isNonEmptyString(action.label) ? action.label : "Do";
