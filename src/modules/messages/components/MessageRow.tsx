...
export const MessageRow = React.memo(function MessageRow({
  ...
  const stackSpacing = index === 0 || showDateDivider ? "mt-0" : startsGroup ? "mt-2.5" : "mt-1";
  ...
  const createdAtTime = formatMessageTime(message.createdAt);
      ...
      <div className={`flex w-full flex-col gap-0.5 ${stackSpacing}`}>
            ...
            <div className="inline-flex flex-wrap items-center gap-1 rounded-full bg-muted px-1.5 py-0.5 text-xs text-foreground">
              {reactions.map((reaction) => (
                <button
                  ...
                  className={`inline-flex h-8 items-center gap-1 rounded-full px-2 py-1 text-[12px] font-medium transition hover:bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
        ...
        {activeActionMessageId === message.id && !isDeletedMessage && (
          <div className={`mt-1 flex w-full ${isSelf ? "justify-end pr-6" : "justify-start pl-6"}`}>
            <div
              className="soft-popover inline-flex max-w-[min(360px,92vw)] flex-col gap-2 p-2.5 text-foreground select-none"
              data-message-action-scope={message.id}
            >
              <div className="flex items-center justify-center gap-1.5">
                {REACTION_EMOJIS.map((emoji) => (
                  <button
                    ...
                    className="inline-flex h-9 w-9 items-center justify-center rounded-full transition hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                  >
              ...
              <div
                className={`flex flex-wrap items-center gap-1.5 ${isSelf ? "justify-end" : "justify-start"}`}
              >
                {text ? (
                  <button
                    type="button"
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full bg-muted/60 px-3 py-1.5 text-[12px] font-medium text-foreground transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                  >
                ...
                {isSelf ? (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full bg-destructive/10 px-3 py-1.5 text-[12px] font-medium text-destructive transition hover:bg-destructive/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive/50 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                  >
        ...
        {showDeliveryStatus && deliveryStatus && (
          <div
            className={`mt-0.5 flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground ${
          ...
          >
            {deliveryStatus.status === "failed" ? (
              <>
                ...
                <span>Failed · {createdAtTime}</span>
                <button
                  type="button"
                  className="text-destructive underline underline-offset-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive/50 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              ...
              <>
                ...
                <span>Sending… · {createdAtTime}</span>
              </>
            ) : deliveryStatus.status === "sent" ? (
              <>
                <Check className="h-3 w-3" aria-hidden />
                <span>Sent · {createdAtTime}</span>
              </>
            ) : deliveryStatus.status === "delivered" ? (
              <>
                <CheckCheck className="h-3 w-3" aria-hidden />
                <span>Delivered · {createdAtTime}</span>
              </>
            ) : (
              <>
                <CheckCheck className="h-3 w-3" aria-hidden />
