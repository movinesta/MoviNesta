...
const ensureEntry = (conversationId: string): Entry => {
  ...
  // Performance: keep the number of postgres_changes subscriptions low.
  // One subscription per table with event='*', then dispatch in-process.
  // This cuts down on Realtime multiplexing and server-side filtering overhead.
  channel
    // Messages
    .on(
      "postgres_changes",
      ...
      (payload) => {
        ...
        } else if (typed.eventType === "UPDATE") {
    ...
    .on(
      ...
      (payload) => {
        ...
        if (typed.eventType === "INSERT" || typed.eventType === "UPDATE") {
    ...
    .on(
      ...
      (payload) => {
        ...
        } else if (typed.eventType === "DELETE") {
