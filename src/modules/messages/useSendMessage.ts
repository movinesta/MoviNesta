  ...
  return useMutation<ConversationMessage, Error, SendMessageArgs, SendMessageContext>({
    mutationFn: async ({ text, attachmentPath, clientId }: SendMessageArgs) => {
      ...
      try {
        ...
        console.error("[useSendMessage] Failed to update conversation timestamp", err);
      }
    ...
    onMutate: async (variables) => {
      ...
      const { payload, trimmed } = buildMessagePayload(text, attachmentPath ?? null, effectiveClientId);
      if (!payload) {
        throw new Error("Cannot send an empty message.");
      }
      ...
      const previous = queryClient.getQueryData<InfiniteData<ConversationMessagesPage>>(messagesKey);
      ...
      return {
        ...
        tempId,
        optimistic,
        payload: { text: trimmed, attachmentPath: attachmentPath ?? null, clientId: effectiveClientId },
      };
    },
    onError: (error, _variables, context) => {
      ...
      const { previous, optimistic, payload, tempId } = context ?? { previous: undefined, tempId: null };
      const messagesKey = conversationMessagesQueryKey(conversationId);
