...
type SendMessageContext = {
  ...
  /** True when the optimistic createdAt was anchored to a cached server timestamp. */
...
export const useSendMessage = (
  ...
  options?: {
    onFailed?: (tempId: string, payload: FailedMessagePayload, error: Error) => void;
  ...
  return useMutation<ConversationMessage, Error, SendMessageArgs, SendMessageContext>({
    mutationFn: async ({ text, attachmentPath, clientId }: SendMessageArgs) => {
      ...
      const row: ConversationMessage = {
        ...
        meta: ((data as any).meta as unknown) ?? null,
      };
    ...
    onMutate: async (variables) => {
      ...
      // Make optimistic timestamps monotonic so the message reliably appears at the end of the list.
      // IMPORTANT: do *not* use `Date.now()` when we already have a server-derived lastCreatedAt.
      // If the device clock is ahead of the server, using `Date.now()` will push optimistic messages
      // into the future. When the server response arrives (with an earlier created_at), the message
      // can re-sort upwards and it *looks like the list jumps to the top*.
      const cached = queryClient.getQueryData<InfiniteData<ConversationMessagesPage>>(messagesKey);
      const lastCached = cached?.pages?.[cached.pages.length - 1]?.items;
      const lastCreatedAt =
        lastCached && lastCached.length ? lastCached[lastCached.length - 1]?.createdAt : null;
      const lastMs = lastCreatedAt ? new Date(lastCreatedAt).getTime() : NaN;
      const usedServerAnchor = Number.isFinite(lastMs);
      const baseMs = usedServerAnchor ? (lastMs as number) : Date.now();
      const createdAt = new Date(baseMs + 1).toISOString();
    ...
    onError: (error, _variables, context) => {
      ...
      if (tempId && payload && options?.onFailed) {
        options.onFailed(tempId, payload, error);
      }
    },
    onSuccess: (row, _variables, context) => {
      ...
      // Keep the createdAt monotonic with the optimistic placeholder so the message doesn't
      // reorder (and cause a visible scroll jump) when the server response arrives.
      // Also prevents remote messages (server time) from being inserted above your optimistic
      // "future" message and breaking auto-scroll on receive.
      const optimisticCreatedAt = context?.optimistic?.createdAt ?? null;
      if (optimisticCreatedAt && context?.usedServerAnchor) {
        const serverMs = new Date(row.createdAt).getTime();
        const optimisticMs = new Date(optimisticCreatedAt).getTime();
        if (Number.isFinite(serverMs) && Number.isFinite(optimisticMs) && serverMs < optimisticMs) {
          row = { ...row, createdAt: optimisticCreatedAt };
        }
      }
