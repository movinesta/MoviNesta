...
export const useTypingChannel = (params: {
  ...
  const typingInactivityMs = getNumber("ux.typing.inactivity_ms", 3000);
  const typingHeartbeatMs = getNumber("ux.typing.heartbeat_ms", 2000);
  const remoteTypingTtlMs = getNumber("ux.typing.remote_ttl_ms", 5000);
  ...
  const noteLocalInputActivity = useCallback(
    (hasMeaningfulText: boolean) => {
      ...
      if (hasMeaningfulText) {
        ...
        } else if (now - localStateRef.current.lastSentAtMs > typingHeartbeatMs) {
          localStateRef.current.lastSentAtMs = now;
          void broadcastTyping(true);
        }
