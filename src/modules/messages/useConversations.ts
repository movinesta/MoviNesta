...
export const useConversations = () => {
  ...
  return useQuery<ConversationListItem[]>({
    ...
    // Performance: avoid aggressive polling (this is the #1 driver of get_conversation_summaries_v2 calls).
    // We only poll while the tab is visible, and at a slower cadence to scale better pre-launch.
    refetchOnWindowFocus: false,
    refetchOnReconnect: true,
    refetchInterval: userId ? () => (document?.hidden ? false : 60_000) : false,
