...
import {
  messageDeliveryReceiptsQueryKey,
  conversationReadReceiptsQueryKey,
} from "./queryKeys";
import {
...
export const useConversationReadReceipts = (conversationId: string | null) => {
  ...
  const queryKey = conversationReadReceiptsQueryKey(conversationId);
  ...
  const createReadReceiptHandlers = useCallback(() => {
    ...
    const upsert = createConversationScopedUpsertIntoListHandler<ReadReceiptRow, ConversationReadReceipt>({
      conversationId,
      queryClient,
      queryKey,
...
export const useConversationDeliveryReceipts = (
  ...
  const queryKey = messageDeliveryReceiptsQueryKey(conversationId, normalizedMessageIds);
  const { pollWhenRealtimeDown, onRealtimeStatus, refetchOptions } = useRealtimeQueryFallbackOptions(
    conversationId,
  );
  ...
  const createDeliveryReceiptHandlers = useCallback(() => {
    ...
    const upsert = createConversationScopedUpsertIntoListHandler<DeliveryReceiptRow, MessageDeliveryReceipt>({
      conversationId,
      queryClient,
      queryKey,
  ...
  const realtimeConversationId = conversationId && normalizedMessageIds.length > 0 ? conversationId : null;
