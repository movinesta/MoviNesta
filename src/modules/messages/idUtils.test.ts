...
import { TEMP_ID_PREFIX, createRandomTempId, createTimestampTempId, ensureTempId, isTempId, normalizeIdList } from "./idUtils";
...
describe("idUtils", () => {
  ...
  it("normalizes id lists", () => {
    expect(
      normalizeIdList([" a ", "", "b", "a", null, undefined, "temp-1"], { excludeTemp: true, sort: true }),
    ).toEqual(["a", "b"]);
  });
});
