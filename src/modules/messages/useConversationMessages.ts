...
const quoteOrValue = (value: string) => {
  // PostgREST `.or()` filters are expressed as a mini-language inside a query param.
  // Values that include special characters (notably `:` / `+` in ISO timestamps) should be quoted.
  // If the value itself contains quotes, escape them.
...
const buildOlderThanFilter = (cursor: ConversationMessagesCursor) => {
  ...
  const createdAt = quoteOrValue(cursor.createdAt);
  const id = cursor.id;
