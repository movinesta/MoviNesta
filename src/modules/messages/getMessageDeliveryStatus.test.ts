...
import type { ConversationMessage, MessageDeliveryReceipt, ConversationReadReceipt } from "./messageModel";
...
describe("getMessageDeliveryStatus", () => {
  it("returns sent when only some recipients have delivery receipts in a group", () => {
    ...
    const deliveryReceipts: MessageDeliveryReceipt[] = [
      { id: "r1", conversationId: "c1", messageId: "m1", userId: "u2", deliveredAt: "2025-01-01T00:00:05Z" },
    ];

    const status = getMessageDeliveryStatus(message, conversation, deliveryReceipts, [], selfId, {});
  ...
  it("returns delivered when all recipients have delivery receipts (dm and group)", () => {
    ...
    const deliveryReceipts: MessageDeliveryReceipt[] = [
      { id: "r1", conversationId: "c1", messageId: "m1", userId: "u2", deliveredAt: "2025-01-01T00:00:05Z" },
      { id: "r2", conversationId: "c1", messageId: "m1", userId: "u3", deliveredAt: "2025-01-01T00:00:06Z" },
    ];
  ...
  it("returns seen only when all recipients have read receipts at/after the message time", () => {
    ...
    const partialRead: ConversationReadReceipt[] = [{ userId: "u2", lastReadAt: "2025-01-01T00:00:10Z" }];
    const status1 = getMessageDeliveryStatus(message, conversation, deliveryReceipts, partialRead, selfId, {});
    expect(status1?.status).toBe("delivered");

    const allRead: ConversationReadReceipt[] = [
      { userId: "u2", lastReadAt: "2025-01-01T00:00:10Z" },
      { userId: "u3", lastReadAt: "2025-01-01T00:00:12Z" },
    ];
    const status2 = getMessageDeliveryStatus(message, conversation, deliveryReceipts, allRead, selfId, {});
  ...
  it("uses the latest read receipt timestamp when duplicates exist for a user", () => {
    ...
    const readReceipts: ConversationReadReceipt[] = [
      { userId: "u2", lastReadAt: "2024-12-31T23:59:59Z" },
      { userId: "u2", lastReadAt: "2025-01-01T00:00:10Z" },
    ];

    const status = getMessageDeliveryStatus(message, conversation, deliveryReceipts, readReceipts, selfId, {});
