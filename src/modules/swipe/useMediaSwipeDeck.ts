import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { useMutation, useQueryClient } from "@tanstack/react-query";

import { qk } from "@/lib/queryKeys";
import { tmdbImageUrl } from "@/lib/tmdb";
import { getOrFetchExperimentAssignments } from "@/lib/experiments";
import { safeLocalStorageGetItem, safeLocalStorageSetItem } from "@/lib/storage";

import type { SwipeDirection } from "./useSwipeDeck";
import {
  fetchMediaSwipeDeck,
  getOrCreateMediaSwipeSessionId,
  getOrCreateSwipeDeckSeedForMode,
  rotateSwipeDeckSeedForMode,
  sendMediaSwipeEventsMulti,
  type MediaSwipeCard,
  type MediaSwipeDeckMode,
  type MediaSwipeEventType,
  uuidv4Fallback,
} from "./mediaSwipeApi";
import { enqueueSwipeEvent, loadQueuedEvents, removeQueuedEvents } from "./eventQueue";

/**
 * Swipe Brain v3 deck hook — performance-tuned.
 *
 * Key optimizations:
 * - Prefetch only a small number of upcoming images (not the whole deck).
 * - Use smaller TMDB image size for swipe cards to reduce memory/CPU.
 * - Avoid spamming events:
 *   - impression is sent once per card per deck
 *   - dwell is debounced + thresholded
 * - Optional: disable heavy query invalidations (default OFF).
 */

export type MediaSwipeDeckKind = "for-you" | "from-friends" | "trending" | "popular";
export type MediaSwipeDeckKindOrCombined = MediaSwipeDeckKind | "combined";

export type MediaSwipeCardUI = MediaSwipeCard & {
  id: string; // media_items.id
  deckId: string; // deck id returned by edge
  recRequestId: string | null; // request trace id returned by edge
  position: number; // index inside deck response (0-based)
  dedupeKey?: string | null; // stable served key from edge (rec_request_id:position:media_item_id)
  source: MediaSwipeDeckKind | "explore" | "combined" | null;
};

type DeckStatus = "idle" | "loading" | "ready" | "exhausted" | "error";

type DeckState = {
  status: DeckStatus;
  cards: MediaSwipeCardUI[];
  errorMessage?: string | null;
};

type SwipeEventPayload = {
  card: MediaSwipeCardUI;
  direction: SwipeDirection;

  /**
   * Optional fine-grained action intent.
   * Examples: "more_like_this", "not_interested".
   * This is recorded inside media_events.payload and used for filtering outcomes.
   */
  action?: string | null;

  // NOTE: rating/watchlist are now tracked via explicit events (eventType: "rating" / "watchlist")
  // to avoid inflating telemetry. These are intentionally NOT sent on swipe events anymore.
  rating0_10?: number | null;
  inWatchlist?: boolean | null;
};

type UseMediaSwipeDeckOptions = {
  limit?: number;
  seed?: string | null;

  /** Filter content kind server-side (movie/series/anime). */
  kindFilter?: "movie" | "series" | "anime" | null;

  /** How many images to prefetch in idle time (default 6). */
  prefetchImages?: number;

  /** Dwell threshold before sending (ms). Default 1200ms. */
  dwellThresholdMs?: number;

  /** Debounce dwell events per card (ms). Default 8000ms. */
  dwellDebounceMs?: number;

  /** Keep legacy invalidations (can be expensive). Default false. */
  invalidateHomeQueries?: boolean;

  /** Optional server-side filtering: drop cards with IMDb rating lower than this (0-10). */
  minImdbRating?: number | null;

  /** Optional server-side filtering: keep cards matching any of these genres. */
  genresAny?: string[] | null;
};

const SWIPE_EVENTS_BACKOFF_STORAGE_KEY = "mn_swipe_events_backoff_v1";

type SwipeEventsBackoffState = { failCount: number; nextAt: number; updatedAt: number };

function readSwipeEventsBackoffState(): SwipeEventsBackoffState {
  try {
    const raw = safeLocalStorageGetItem(SWIPE_EVENTS_BACKOFF_STORAGE_KEY);
    if (!raw) return { failCount: 0, nextAt: 0, updatedAt: 0 };
    const parsed = JSON.parse(raw) as Partial<SwipeEventsBackoffState>;
    return {
      failCount: Number(parsed.failCount ?? 0) || 0,
      nextAt: Number(parsed.nextAt ?? 0) || 0,
      updatedAt: Number(parsed.updatedAt ?? 0) || 0,
    };
  } catch {
    return { failCount: 0, nextAt: 0, updatedAt: 0 };
  }
}

function writeSwipeEventsBackoffState(state: SwipeEventsBackoffState) {
  try {
    safeLocalStorageSetItem(SWIPE_EVENTS_BACKOFF_STORAGE_KEY, JSON.stringify(state));
  } catch {
    // ignore storage errors
  }
}

function resetSwipeEventsBackoff() {
  writeSwipeEventsBackoffState({ failCount: 0, nextAt: 0, updatedAt: Date.now() });
}

function bumpSwipeEventsBackoff() {
  const prev = readSwipeEventsBackoffState();
  const nextFail = Math.min(10, Math.max(0, prev.failCount) + 1);
  // Exponential backoff with FULL jitter (random in [0, cap]) to avoid synchronized retries.
  const base = 1000;
  const cap = 60_000;
  const exp = Math.min(cap, base * (2 ** (nextFail - 1)));
  const jitter = Math.floor(Math.random() * exp);
  const delay = Math.max(250, jitter);
  writeSwipeEventsBackoffState({ failCount: nextFail, nextAt: Date.now() + delay, updatedAt: Date.now() });
}

function getSwipeEventsBackoffWaitMs(): number {
  const st = readSwipeEventsBackoffState();
  const now = Date.now();
  if (!st.nextAt || st.nextAt <= now) return 0;
  return st.nextAt - now;
}

function mapKindToMode(kind: MediaSwipeDeckKindOrCombined): MediaSwipeDeckMode {
  if (kind === "trending" || kind === "popular") return "trending";
  if (kind === "from-friends") return "friends";
  if (kind === "combined") return "combined";
  return "for_you";
}

function mapBackendSource(src: string | null | undefined): MediaSwipeCardUI["source"] {
  if (!src) return null;
  const normalized = src.replace("-", "_");
  if (normalized === "for_you") return "for-you";
  if (normalized === "friends") return "from-friends";
  if (normalized === "trending") return "trending";
  if (normalized === "popular") return "popular";
  if (normalized === "explore") return "explore";
  if (normalized === "combined") return "combined";
  return null;
}

function mapDirectionToEventType(direction: SwipeDirection): MediaSwipeEventType {
  if (direction === "like") return "like";
  if (direction === "dislike") return "dislike";
  return "skip";
}

function nextClientEventId(): string {
  if (typeof crypto !== "undefined" && "randomUUID" in crypto) return crypto.randomUUID();
  return uuidv4Fallback();
}

function normalizeCard(card: MediaSwipeCardUI): MediaSwipeCardUI | null {
  // Prefer OMDb poster URLs; fall back to TMDB so users never see an empty deck.
  const omdbPoster = typeof card.posterUrl === "string" ? card.posterUrl.trim() : "";
  const posterUrl =
    (omdbPoster && omdbPoster.toLowerCase() !== "n/a" ? omdbPoster : "") ||
    (card.tmdbPosterPath ? (tmdbImageUrl(card.tmdbPosterPath, "w500") ?? "") : "") ||
    (card.tmdbBackdropPath ? (tmdbImageUrl(card.tmdbBackdropPath, "w780") ?? "") : "");

  if (!posterUrl) return null;

  const title = (card.title ?? "").trim() || "Untitled";
  return { ...card, posterUrl, title };
}

function scheduleAssetPrefetch(incoming: MediaSwipeCardUI[], max: number) {
  if (typeof window === "undefined") return;
  if (max <= 0) return;

  const conn = (navigator as any).connection;
  if (conn?.saveData) return; // be nice on data-saver

  const idle = (window as typeof window & { requestIdleCallback?: typeof requestIdleCallback })
    .requestIdleCallback;
  const runner = idle ?? ((cb: () => void) => window.setTimeout(cb, 300));

  const slice = incoming.slice(0, max);

  runner(() => {
    for (const card of slice) {
      if (!card.posterUrl) continue;
      const img = new Image();
      img.decoding = "async";
      img.loading = "lazy";
      img.src = card.posterUrl;
    }
  });
}

export function useMediaSwipeDeck(
  kind: MediaSwipeDeckKindOrCombined,
  options?: UseMediaSwipeDeckOptions,
) {
  const limit = options?.limit ?? 60;
  const prefetchImages = options?.prefetchImages ?? 6;
  const dwellThresholdMs = options?.dwellThresholdMs ?? 1200;
  const dwellDebounceMs = options?.dwellDebounceMs ?? 8000;
  const invalidateHomeQueries = options?.invalidateHomeQueries ?? false;

  const queryClient = useQueryClient();

  const bufferRef = useRef<any[]>([]);
  const flushTimerRef = useRef<number | null>(null);
  const backoffQueueTimerRef = useRef<number | null>(null);
  const backoffBufferedTimerRef = useRef<number | null>(null);

  const inFlightMapRef = useRef<Map<string, any>>(new Map());

  // Flush any queued swipe events (offline/failed) on mount and when the browser comes online.
  useEffect(() => {
    let cancelled = false;

    const flush = async () => {
      if (cancelled) return;
      const waitMs = getSwipeEventsBackoffWaitMs();
      if (waitMs > 0) {
        if (backoffQueueTimerRef.current == null) {
          backoffQueueTimerRef.current = window.setTimeout(() => {
            backoffQueueTimerRef.current = null;
            void flush();
          }, Math.min(waitMs + 50, 65_000));
        }
        return;
      }
      const queued = loadQueuedEvents();
      if (!queued.length) return;

      const sent: string[] = [];
      const chunkSize = 25;

      for (let i = 0; i < queued.length; i += chunkSize) {
        if (cancelled) return;
        const chunk = queued.slice(i, i + chunkSize);

        try {
          const items = chunk.map((ev) => ({
            sessionId: ev.sessionId,
            deckId: (ev as any).deckId ?? null,
            recRequestId: (ev as any).recRequestId ?? null,
            position: (ev as any).position ?? null,
            dedupeKey: (ev as any).dedupeKey ?? (ev as any).dedupe_key ?? null,
            mediaItemId: ev.mediaItemId,
            source: (ev as any).source ?? null,
            events: [
              {
                eventType: (ev as any).eventType,
                dwellMs: (ev as any).dwellMs ?? null,
                rating0_10: (ev as any).rating0_10 ?? null,
                inWatchlist: (ev as any).inWatchlist ?? null,
                clientEventId: (ev as any).clientEventId ?? null,
                payload: (ev as any).payload ?? null,
              },
            ],
          }));

          const res = await sendMediaSwipeEventsMulti(items);
          if (res.ok) {
            const removeIds = [
              ...(res.acceptedClientEventIds ?? []),
              ...(res.rejectedClientEventIds ?? []),
            ].filter(Boolean);
            if (removeIds.length) sent.push(...removeIds);
            // If the server indicated any retryable events, stop flushing further chunks
            // to avoid hammering (we already shrank the queue by removing accepted/rejected).
            if (res.shouldRetry) {
              bumpSwipeEventsBackoff();
              break;
            }
            // Successful chunk without retry -> reset backoff.
            resetSwipeEventsBackoff();
          } else {
            bumpSwipeEventsBackoff();
            break;
          }
        } catch {
          bumpSwipeEventsBackoff();
          break;
        }
      }

      if (sent.length) removeQueuedEvents(sent);
    };

    void flush();

    const onOnline = () => void flush();
    window.addEventListener("online", onOnline);
    return () => {
      cancelled = true;
      window.removeEventListener("online", onOnline);
    };
  }, []);

  // Experiment assignments are optional. If present, we attach them to event payloads
  // so offline analysis can slice metrics by variant.
  const experimentsRef = useRef<Record<string, string>>({});

  const flushBuffered = useCallback(async (forceAll?: boolean) => {
    const waitMs = getSwipeEventsBackoffWaitMs();
    if (waitMs > 0) {
      if (backoffBufferedTimerRef.current == null) {
        backoffBufferedTimerRef.current = window.setTimeout(() => {
          backoffBufferedTimerRef.current = null;
          void flushBuffered(forceAll);
        }, Math.min(waitMs + 50, 65_000));
      }
      return;
    }

    const buf = bufferRef.current;
    if (!buf.length) return;

    const chunkSize = 25;
    const toSend = forceAll
      ? buf.splice(0, buf.length)
      : buf.splice(0, Math.min(chunkSize, buf.length));

    // Track in-flight events so they can be persisted on hard unload.
    for (const ev of toSend as any[]) {
      if (!ev.clientEventId) ev.clientEventId = nextClientEventId();
      inFlightMapRef.current.set(ev.clientEventId, ev);
    }

    const items = toSend.map((ev: any) => ({
      sessionId: ev.sessionId,
      deckId: ev.deckId ?? null,
      recRequestId: ev.recRequestId ?? null,
      position: ev.position ?? null,
      dedupeKey: ev.dedupeKey ?? (ev as any).dedupe_key ?? null,
      mediaItemId: ev.mediaItemId,
      source: ev.source ?? null,
      events: [
        {
          eventType: ev.eventType,
          dwellMs: ev.dwellMs ?? null,
          rating0_10: ev.rating0_10 ?? null,
          inWatchlist: ev.inWatchlist ?? null,
          clientEventId: ev.clientEventId ?? null,
          payload: ev.payload ?? null,
        },
      ],
    }));

    try {
      const res = await sendMediaSwipeEventsMulti(items);

      if (!res.ok) {
        bumpSwipeEventsBackoff();
        for (const ev of toSend) enqueueSwipeEvent(ev);
      } else {
        const acceptedIds = new Set(res.acceptedClientEventIds ?? []);
        const rejectedIds = new Set(res.rejectedClientEventIds ?? []);
        const retryIds = new Set(res.retryClientEventIds ?? []);

        for (const ev of toSend as any[]) {
          const id = ev.clientEventId as string | null | undefined;
          if (!id) {
            // Should not happen (we generate ids), but never drop unknown events.
            enqueueSwipeEvent(ev);
            continue;
          }
          if (retryIds.has(id)) {
            enqueueSwipeEvent(ev);
            continue;
          }
          if (acceptedIds.has(id) || rejectedIds.has(id)) continue;
          // If server didn't classify this id, retry to be safe.
          enqueueSwipeEvent(ev);
        }
      }

      if (res.ok && res.shouldRetry) bumpSwipeEventsBackoff();
      if (res.ok && !res.shouldRetry) resetSwipeEventsBackoff();

      // Clear in-flight markers regardless of success; failures are persisted to queue.
      for (const ev of toSend as any[])
        if (ev.clientEventId) inFlightMapRef.current.delete(ev.clientEventId);
    } catch {
      bumpSwipeEventsBackoff();
      for (const ev of toSend) enqueueSwipeEvent(ev);
      for (const ev of toSend as any[])
        if (ev.clientEventId) inFlightMapRef.current.delete(ev.clientEventId);
    }
  }, []);

  const bufferSingleEvent = useCallback(
    (ev: any) => {
      // Ensure clientEventId for idempotency + queue cleanup.
      if (!ev.clientEventId) ev.clientEventId = nextClientEventId();
      bufferRef.current.push(ev);
      if (bufferRef.current.length >= 25) {
        void flushBuffered();
        return;
      }
      if (flushTimerRef.current == null) {
        flushTimerRef.current = window.setTimeout(() => {
          flushTimerRef.current = null;
          void flushBuffered();
        }, 1000);
      }
    },
    [flushBuffered],
  );

  const bufferEvent = useCallback(
    (ev: any) => {
      // Support legacy/batch payloads: { ...base, events: [{eventType,...}, ...] }
      if (Array.isArray(ev?.events)) {
        const { events, ...base } = ev as any;
        const basePayload =
          typeof (base as any).payload === "object" && (base as any).payload ? (base as any).payload : null;
        for (const e of events) {
          if (!e) continue;
          const evPayload = typeof e?.payload === "object" && e.payload ? e.payload : null;
          bufferSingleEvent({
            ...base,
            ...e,
            // Merge base payload (e.g., experiments) into each event payload.
            payload: basePayload || evPayload ? { ...(basePayload ?? {}), ...(evPayload ?? {}) } : null,
            // Ensure each nested event gets its own id.
            clientEventId: e?.clientEventId ?? null,
          });
        }
        return;
      }

      bufferSingleEvent(ev);
    },
    [bufferSingleEvent],
  );

  const drainPendingToQueue = useCallback(() => {
    // Cancel scheduled flush and persist everything we might otherwise lose.
    if (flushTimerRef.current != null) {
      window.clearTimeout(flushTimerRef.current);
      flushTimerRef.current = null;
    }

    // Persist any buffered events.
    const buf = bufferRef.current;
    if (buf.length) {
      const toDrain = buf.splice(0, buf.length);
      for (const ev of toDrain) enqueueSwipeEvent(ev);
    }

    // Persist any in-flight events (e.g., during hard unload). Safe due to server-side idempotency.
    const inflight = inFlightMapRef.current;
    if (inflight.size) {
      for (const ev of inflight.values()) enqueueSwipeEvent(ev);
      inflight.clear();
    }
  }, []);

  // Ensure we don't lose buffered/in-flight events on hard unload.
  useEffect(() => {
    const onPageHide = () => drainPendingToQueue();
    const onBeforeUnload = () => drainPendingToQueue();
    window.addEventListener("pagehide", onPageHide);
    window.addEventListener("beforeunload", onBeforeUnload);
    return () => {
      window.removeEventListener("pagehide", onPageHide);
      window.removeEventListener("beforeunload", onBeforeUnload);
      // Also persist on unmount as a safety net.
      drainPendingToQueue();
    };
  }, [drainPendingToQueue]);

  // Best-effort flush when the tab becomes hidden (keeps queue smaller).
  useEffect(() => {
    const onVis = () => {
      if (document.visibilityState === "hidden") {
        void flushBuffered(true);
      }
    };
    document.addEventListener("visibilitychange", onVis);
    return () => document.removeEventListener("visibilitychange", onVis);
  }, [flushBuffered]);

  const sessionId = useMemo(() => getOrCreateMediaSwipeSessionId(), []);
  const mode = useMemo(() => mapKindToMode(kind), [kind]);

  // Fetch experiment assignments once per mount (best-effort).
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const assignments = await getOrFetchExperimentAssignments(
          [
            "swipe_pipeline_v1",
            "swipe_rerank_v1",
            "swipe_diversity_v1",
            "swipe_mix_v1",
            "swipe_cf_v1",
            "swipe_blend_v1",
          ],
          {
            ttlMs: 1000 * 60 * 60 * 6, // 6h
          },
        );
        if (!cancelled) experimentsRef.current = assignments;
      } catch {
        // ignore
      }
    })();
    return () => {
      cancelled = true;
    };
  }, []);

  const seenIdsRef = useRef<Set<string>>(new Set());
  const fetchingRef = useRef(false);
  const pageRef = useRef(0);
  const baseSeedRef = useRef<string | null>(null);

  // Avoid spamming impression/dwell
  const impressedRef = useRef<Set<string>>(new Set()); // key = `${deckId}:${mediaId}`
  const lastDwellSentAtRef = useRef<Map<string, number>>(new Map()); // key = `${deckId}:${mediaId}` -> ts

  const [state, setState] = useState<DeckState>({
    status: "idle",
    cards: [],
    errorMessage: null,
  });

  const [eventError, setEventError] = useState<{ payload: any; message: string } | null>(null);

  const setStateSafe = useCallback((updater: DeckState | ((prev: DeckState) => DeckState)) => {
    setState((prev) => (typeof updater === "function" ? (updater as any)(prev) : updater));
  }, []);

  const appendCards = useCallback(
    (incoming: MediaSwipeCardUI[]): number => {
      if (!incoming.length) return 0;

      const seen = seenIdsRef.current;
      const fresh: MediaSwipeCardUI[] = [];

      for (const raw of incoming) {
        if (seen.has(raw.id)) continue;
        // Normalize BEFORE marking as seen so we don't poison the seen set
        // with cards we intentionally drop. Still mark dropped ones as seen
        // to avoid tight retry loops within a session.
        const normalized = normalizeCard(raw);
        seen.add(raw.id);
        if (!normalized) continue;
        fresh.push(normalized);
      }

      if (!fresh.length) return 0;

      setStateSafe((prev) => {
        const next = {
          ...prev,
          status: "ready" as const,
          cards: [...prev.cards, ...fresh],
          errorMessage: null,
        };
        scheduleAssetPrefetch(fresh, prefetchImages);
        return next;
      });

      return fresh.length;
    },
    [prefetchImages, setStateSafe],
  );

  const fetchBatch = useCallback(
    async (batchSize = limit) => {
      if (fetchingRef.current) return;
      fetchingRef.current = true;

      setStateSafe((prev) => ({
        ...prev,
        status: prev.cards.length ? "ready" : "loading",
        errorMessage: null,
      }));

      try {
        const resolvedKindFilter = options?.kindFilter ?? null;

        // Use a stable base seed (sessionStorage) and add a page suffix so fetchMore doesn't repeat.
        const baseSeed =
          options?.seed ?? getOrCreateSwipeDeckSeedForMode(sessionId, mode, resolvedKindFilter);
        if (baseSeedRef.current !== baseSeed) {
          baseSeedRef.current = baseSeed;
          pageRef.current = 0;
        }
        // Some sources can return cards that we later drop (e.g., missing OMDb poster).
        // If that happens and we don't add anything, the UI can get stuck in "loading".
        // We retry a small number of times with the next page seed, then gracefully
        // fall back to "exhausted" so the user sees the empty state (not a spinner).
        const MAX_FILL_ATTEMPTS = 3;
        let attempt = 0;
        let added = 0;

        while (attempt < MAX_FILL_ATTEMPTS && added === 0) {
          // Advance the page suffix each attempt so retries don't keep returning
          // the same (potentially unrenderable) candidates.
          const seedToUse = `${baseSeed}:${pageRef.current}`;
          pageRef.current += 1;
          const resp = await fetchMediaSwipeDeck(
            {
              sessionId,
              mode,
              kindFilter: resolvedKindFilter ?? undefined,
              limit: batchSize,
              seed: seedToUse,
              minImdbRating: options?.minImdbRating ?? null,
              genresAny: options?.genresAny ?? null,
              experiments: Object.keys(experimentsRef.current ?? {}).length
                ? experimentsRef.current
                : null,
            },
            { timeoutMs: 25000 },
          );

          const deckId = resp.deckId;
          const recRequestId = resp.recRequestId ?? null;
          const incoming = (resp.cards ?? []).map((c, idx) => ({
            ...c,
            id: c.mediaItemId,
            deckId,
            recRequestId,
            // Prefer server-stamped position so client-side filtering does not shift indices.
            position: typeof (c as any).position === "number" ? (c as any).position : idx,
            dedupeKey: (c as any).dedupeKey ?? (c as any).dedupe_key ?? null,
            source: mapBackendSource((c as any).source ?? kind),
          })) as MediaSwipeCardUI[];

          if (!incoming.length) {
            setStateSafe((prev) => ({
              ...prev,
              status: prev.cards.length ? "ready" : "exhausted",
              errorMessage: null,
            }));
            return;
          }

          added = appendCards(incoming);
          if (added > 0) break;

          attempt += 1;

          // If everything was filtered out, advance the seed/page and try again.
          // (fetchBatch() already increments pageRef.current, so this will naturally
          // move forward on the next user action as well.)
          console.warn(
            "[useMediaSwipeDeck] filtered out all cards (missing OMDb poster?). Retrying...",
            { attempt, mode, kind, batchSize },
          );
        }

        if (added === 0) {
          // Nothing usable after retries: show the empty state.
          setStateSafe((prev) => ({
            ...prev,
            status: prev.cards.length ? "ready" : "exhausted",
            errorMessage: null,
          }));
        }
      } catch (err) {
        const message =
          err instanceof Error
            ? err.message
            : typeof err === "string"
              ? err
              : "We couldn't load swipe cards.";
        setStateSafe((prev) => ({ ...prev, status: "error", errorMessage: message }));
      } finally {
        fetchingRef.current = false;
      }
    },
    [
      appendCards,
      limit,
      mode,
      options?.kindFilter,
      options?.minImdbRating,
      JSON.stringify(options?.genresAny ?? []),
      options?.seed,
      sessionId,
      setStateSafe,
    ],
  );

  useEffect(() => {
    // reset when kind changes
    seenIdsRef.current = new Set();
    impressedRef.current = new Set();
    lastDwellSentAtRef.current = new Map();
    fetchingRef.current = false;

    setStateSafe({ status: "loading", cards: [], errorMessage: null });
    fetchBatch(limit);
  }, [
    fetchBatch,
    kind,
    limit,
    options?.kindFilter,
    options?.minImdbRating,
    JSON.stringify(options?.genresAny ?? []),
    setStateSafe,
  ]);

  const refresh = useCallback(() => {
    // If the caller did not force a specific seed, rotate the shared mode seed
    // so a manual refresh produces a new deck (and still shares seed with prefetch).
    if (options?.seed == null) {
      rotateSwipeDeckSeedForMode(sessionId, mode, options?.kindFilter ?? null);
    }
    pageRef.current = 0;
    seenIdsRef.current = new Set();
    impressedRef.current = new Set();
    lastDwellSentAtRef.current = new Map();
    fetchingRef.current = false;

    setStateSafe({ status: "loading", cards: [], errorMessage: null });
    fetchBatch(limit);
  }, [
    fetchBatch,
    limit,
    mode,
    options?.kindFilter,
    options?.minImdbRating,
    JSON.stringify(options?.genresAny ?? []),
    options?.seed,
    sessionId,
    setStateSafe,
  ]);

  const trimConsumed = useCallback(
    (count: number) => {
      if (count <= 0) return;
      setStateSafe((prev) => ({
        ...prev,
        cards: prev.cards.slice(Math.min(count, prev.cards.length)),
      }));
    },
    [setStateSafe],
  );

  // --- Events ---
  const eventMutation = useMutation({
    mutationFn: async (payload: any) => {
      bufferEvent(payload);
      return { ok: true } as any;
    },
    onError: (error, variables) => {
      const message =
        error instanceof Error
          ? error.message
          : typeof error === "string"
            ? error
            : "We couldn't save that action.";
      enqueueSwipeEvent(variables);
      setEventError({ payload: variables, message });
    },
    onSuccess: () => {
      setEventError(null);

      if (invalidateHomeQueries) {
        queryClient.invalidateQueries({ queryKey: qk.homeFeed(null) });
        queryClient.invalidateQueries({ queryKey: qk.homeForYou(null) });
      }
    },
  });

  const trackImpression = useCallback(
    (card: MediaSwipeCardUI | undefined, positionOverride?: number) => {
      if (!card) return;

      const key = `${card.deckId}:${card.id}`;
      if (impressedRef.current.has(key)) return;
      impressedRef.current.add(key);

      eventMutation.mutate({
        sessionId,
        deckId: card.deckId,
        recRequestId: card.recRequestId ?? null,
        position: positionOverride ?? card.position,
        dedupeKey: card.dedupeKey ?? null,
        mediaItemId: card.id,
        eventType: "impression" satisfies MediaSwipeEventType,
        source: card.source ?? null,
        clientEventId: nextClientEventId(),
        payload: (() => {
          const p: Record<string, unknown> = {};
          if (Object.keys(experimentsRef.current).length) p.experiments = experimentsRef.current;
          const why = typeof card.why === "string" ? card.why.trim() : "";
          if (why) p.why = why;
          return Object.keys(p).length ? p : null;
        })(),
      });
    },
    [eventMutation, sessionId],
  );

  const trackDwell = useCallback(
    (card: MediaSwipeCardUI | undefined, dwellMs: number, positionOverride?: number) => {
      if (!card) return;
      if (!Number.isFinite(dwellMs) || dwellMs < dwellThresholdMs) return;

      const key = `${card.deckId}:${card.id}`;
      const now = Date.now();
      const last = lastDwellSentAtRef.current.get(key) ?? 0;
      if (now - last < dwellDebounceMs) return;
      lastDwellSentAtRef.current.set(key, now);

      eventMutation.mutate({
        sessionId,
        deckId: card.deckId,
        recRequestId: card.recRequestId ?? null,
        position: positionOverride ?? card.position,
        dedupeKey: card.dedupeKey ?? null,
        mediaItemId: card.id,
        eventType: "dwell" satisfies MediaSwipeEventType,
        dwellMs: Math.round(dwellMs),
        source: card.source ?? null,
        clientEventId: nextClientEventId(),
        payload: (() => {
          const p: Record<string, unknown> = {};
          if (Object.keys(experimentsRef.current).length) p.experiments = experimentsRef.current;
          const why = typeof card.why === "string" ? card.why.trim() : "";
          if (why) p.why = why;
          return Object.keys(p).length ? p : null;
        })(),
      });
    },
    [dwellDebounceMs, dwellThresholdMs, eventMutation, sessionId],
  );

  const swipe = useCallback(
    (payload: SwipeEventPayload) => {
      const card = payload.card;
      const action = payload.action ?? null;
      const basePayload: Record<string, unknown> = {};
      if (action) basePayload.action = action;

      eventMutation.mutate({
        sessionId,
        deckId: card.deckId,
        recRequestId: card.recRequestId ?? null,
        position: card.position,
        dedupeKey: card.dedupeKey ?? null,
        mediaItemId: card.id,
        eventType: mapDirectionToEventType(payload.direction),
        source: card.source ?? null,
        clientEventId: nextClientEventId(),
        payload:
          Object.keys(experimentsRef.current).length || Object.keys(basePayload).length
            ? {
                ...basePayload,
                ...(Object.keys(experimentsRef.current).length
                  ? { experiments: experimentsRef.current }
                  : {}),
              }
            : null,
      });
    },
    [eventMutation, sessionId],
  );

  const swipeAsync = useCallback(
    async (payload: SwipeEventPayload) => {
      const card = payload.card;
      const action = payload.action ?? null;
      const basePayload: Record<string, unknown> = {};
      if (action) basePayload.action = action;

      return eventMutation.mutateAsync({
        sessionId,
        deckId: card.deckId,
        recRequestId: card.recRequestId ?? null,
        position: card.position,
        dedupeKey: card.dedupeKey ?? null,
        mediaItemId: card.id,
        eventType: mapDirectionToEventType(payload.direction),
        source: card.source ?? null,
        clientEventId: nextClientEventId(),
        payload:
          Object.keys(experimentsRef.current).length || Object.keys(basePayload).length
            ? {
                ...basePayload,
                ...(Object.keys(experimentsRef.current).length
                  ? { experiments: experimentsRef.current }
                  : {}),
              }
            : null,
      });
    },
    [eventMutation, sessionId],
  );

  // Generic event sender (used by rating/watchlist/etc).
  // Best-effort: failures are queued for retry and do not throw when using sendEventAsync().
  const sendEvent = useCallback(
    (payload: any) => {
      const finalPayloadBase = {
        ...payload,
        sessionId: payload.sessionId ?? sessionId,
        clientEventId: payload.clientEventId ?? nextClientEventId(),
      };
      const finalPayload = Object.keys(experimentsRef.current).length
        ? {
            ...finalPayloadBase,
            payload: {
              ...(typeof finalPayloadBase.payload === "object" && finalPayloadBase.payload
                ? finalPayloadBase.payload
                : {}),
              experiments: experimentsRef.current,
            },
          }
        : finalPayloadBase;
      eventMutation.mutate(finalPayload);
    },
    [eventMutation, sessionId],
  );

  const sendEventAsync = useCallback(
    async (payload: any) => {
      const finalPayloadBase = {
        ...payload,
        sessionId: payload.sessionId ?? sessionId,
        clientEventId: payload.clientEventId ?? nextClientEventId(),
      };
      const finalPayload = Object.keys(experimentsRef.current).length
        ? {
            ...finalPayloadBase,
            payload: {
              ...(typeof finalPayloadBase.payload === "object" && finalPayloadBase.payload
                ? finalPayloadBase.payload
                : {}),
              experiments: experimentsRef.current,
            },
          }
        : finalPayloadBase;
      try {
        await eventMutation.mutateAsync(finalPayload);
      } catch {
        // onError already enqueues; keep UI responsive
      }
    },
    [eventMutation, sessionId],
  );

  return {
    sessionId,
    mode,

    cards: state.cards,
    status: state.status,

    isLoading: state.status === "loading",
    isError: state.status === "error",
    isExhausted: state.status === "exhausted",
    deckError: state.errorMessage ?? null,

    fetchMore: fetchBatch,
    refresh,
    trimConsumed,

    trackImpression,
    trackDwell,

    sendEvent,
    sendEventAsync,

    swipe,
    swipeAsync,

    swipeSyncError: eventError?.message ?? null,
    retryFailedSwipe: () => {
      if (eventError) eventMutation.mutate(eventError.payload);
    },
    isRetryingSwipe: eventMutation.isPending && Boolean(eventError),
  };
}
