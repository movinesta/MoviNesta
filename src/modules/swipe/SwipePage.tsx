...
const SwipePage: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const sfx = localStorage.getItem(SWIPE_SFX_STORAGE_KEY);
      const hap = localStorage.getItem(SWIPE_HAPTICS_STORAGE_KEY);
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined") return;
    try {
      localStorage.setItem(SWIPE_SFX_STORAGE_KEY, sfxEnabled ? "1" : "0");
    } catch {
      // ignore
    }
  }, [sfxEnabled]);
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined") return;
    try {
      localStorage.setItem(SWIPE_HAPTICS_STORAGE_KEY, hapticsEnabled ? "1" : "0");
    } catch {
      // ignore
    }
  }, [hapticsEnabled]);
