...
export function PresenceProvider({ children }: { children: React.ReactNode }) {
  ...
  const heartbeatMs = getNumber("ux.presence.heartbeat_ms", 30_000);
  const recomputeMs = getNumber("ux.presence.recompute_ms", 5_000);
  // Keep presence feeling "live" via Realtime heartbeats, but avoid frequent DB writes.
  // Default: at most once every 10 minutes per user.
  const dbTouchMinIntervalMs = getNumber("ux.presence.db_touch_min_interval_ms", 600_000);
  ...
  const touchDbLastSeen = React.useCallback(async () => {
    ...
    // If the tab is hidden, we keep presence "live" via Realtime, but skip DB writes.
    // This avoids unnecessary write load when users leave the app open in the background.
