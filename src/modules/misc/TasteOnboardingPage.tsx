...
const TasteOnboardingPage: React.FC = () => {
  ...
  }, [markOnboardedAndGo, selected, sessionId]);
    ...
    const run = async () => {
      ...
      try {
        const resp = await fetchMediaSwipeDeck(
          { sessionId, mode: "trending", limit: 24, seed: `onb:${new Date().toISOString().slice(0, 10)}` },
          { timeoutMs: 30000 },
        );
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-6 px-4 py-10">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Choose at least {MIN_PICKS}. This helps MoviNesta personalize your “For you” deck immediately.
        </p>
      </div>
            ...
            {results.slice(0, 20).map((r) => {
              ...
              return (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-2xl border border-border bg-card/40 p-2 text-left hover:bg-card/60"
                >
                  {pick.posterUrl ? (
                    <img
                      src={pick.posterUrl}
                  ...
                  <div className="rounded-full bg-background/80 p-1 shadow">
                    {isSelected ? <CheckCircle2 className="h-4 w-4" /> : <Plus className="h-4 w-4" />}
                  </div>
                </button>
              );
