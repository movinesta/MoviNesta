...
type ActivityEventRow = {
  ...
  created_at: string;
  user_id: string;
  event_type: string; // was Database...activity_event_type
  title_id: string | null;
  payload: any;
};

type HomeFeedRow = ActivityEventRow; // Same shape

type PartialTitleRow = ReturnType<typeof mapMediaItemToSummary>;
type PartialProfileRow = {
  id: string;
...
export const fetchHomeFeedPage = async (
  ...
  const { data: eventsData, error: eventsError } = await (supabase.rpc as any)("get_home_feed", {
    p_user_id: userId,
  ...
  ((titlesResult.data as any as MediaItemRow[]) ?? []).forEach((row) => {
