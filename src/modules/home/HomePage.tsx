...
import VerifiedBadge, { type VerifiedBadgeType } from "@/components/VerifiedBadge";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const IconButton: React.FC<{
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="relative inline-flex h-11 w-11 items-center justify-center rounded-full text-foreground transition-colors hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
      aria-label={label}
    >
...
const SectionHeader: React.FC<{
  ...
  <div className="flex items-center justify-between page-pad pb-3">
  ...
  <DropdownMenu>
    <DropdownMenuTrigger asChild>
      <button
        ...
        className="icon-hit-dark bg-black/45 hover:bg-black/55 backdrop-blur"
    ...
    <div className="flex gap-4 rounded-3xl border border-border/40 bg-card/70 p-4 shadow-sm">
      ...
      <div className="min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-1">
              ...
              <VerifiedBadge
                ...
                type={normalizeVerifiedType(item.user.verifiedType)}
                label={item.user.verifiedLabel}
                verifiedAt={item.user.verifiedAt}
                org={item.user.verifiedOrg}
        ...
        <button
          type="button"
          onClick={openTitle}
          className="soft-row-card soft-row-card-interactive mt-3 flex w-full min-h-11 items-center gap-3 p-2 text-left focus-visible:outline-none"
...
const HomePage: React.FC = () => {
  ...
  const {
    ...
    error: combinedError,
  } = useQuery<HomeDeck, Error>({
    queryKey: ["home-v2", "deck", "combined", userId],
    enabled: Boolean(userId),
    queryFn: () => fetchHomeDeck({ sessionId, mode: "combined", limit: 60 }),
    staleTime: 10 * 60_000,
    gcTime: 30 * 60_000,
  ...
  const heroCard = React.useMemo(() => {
    return combinedDeck?.cards?.[0] ?? null;
  }, [combinedDeck?.cards]);
  ...
  const deckCardsBySource = React.useMemo(() => {
    ...
    for (const c of cards) {
      const src = norm((c as any)?.source);
      if (src === "for_you") forYou.push(c);
      if (src === "trending" || src === "popular") trending.push(c);
    }

    return { trending, forYou };
  }, [combinedDeck?.cards]);
  ...
  const trendingItems = React.useMemo(() => {
    ...
    const base = deckCardsBySource.trending.length
      ? deckCardsBySource.trending
      : ((combinedDeck?.cards ?? []) as any[]);

    return base
      .filter((c: any) => c?.mediaItemId && !skip.has(c.mediaItemId))
      .map(cardToPosterItem)
      .filter(Boolean) as PosterItem[];
  }, [deckCardsBySource.trending, combinedDeck?.cards, heroId]);
  ...
  const becauseItems = React.useMemo(() => {
    ...
    const base = deckCardsBySource.forYou.length
      ? deckCardsBySource.forYou
      : deckCardsBySource.trending;
  ...
  }, [deckCardsBySource.forYou, deckCardsBySource.trending, trendingItems, heroId]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-background/85 backdrop-blur full-bleed">
        <div className="mx-auto flex w-full max-w-5xl items-center justify-between page-pad py-3">
          <button
            type="button"
            onClick={() => navigate("/me")}
      ...
      <section className="page-pad">
        <div className="rounded-3xl border border-border/60 bg-card/70 py-4 shadow-sm">
          ...
          <div className="no-scrollbar flex w-full gap-3 overflow-x-auto page-pad py-1">
            <button
              type="button"
      ...
      {userId ? (
        <section className="mt-6">
          ...
          />
          <div className="no-scrollbar flex gap-3 overflow-x-auto page-pad pb-1">
            {isDiaryLoading ? (
      ...
      <section>
        ...
        <div className="page-pad">
          {combinedError ? (
            <div className="rounded-2xl border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive">
              {combinedError.message ? combinedError.message : "Couldn't load trending titles."}
            </div>
          ) : isCombinedLoading ? (
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-6 page-pad pt-6">
          {isCombinedLoading
      ...
      <section>
        ...
        <div className="page-pad">
          {combinedError ? (
            <div className="rounded-2xl border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive">
              {combinedError.message ? combinedError.message : "Couldn't load recommendations."}
            </div>
          ) : isCombinedLoading ? (
            <div className="aspect-[2/3] w-full animate-pulse rounded-3xl bg-muted" />
          ) : becauseFeatured ? (
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-6 page-pad pt-6 pb-2">
          {isCombinedLoading
            ? Array.from({ length: 6 }).map((_, idx) => (
                <div
