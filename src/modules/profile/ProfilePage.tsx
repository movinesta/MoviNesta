...
import { useUserLastActive } from "@/modules/messages/useUserLastActive";
import { formatTimeAgo } from "@/modules/messages/formatTimeAgo";
...
const ProfilePage: React.FC = () => {
  ...
  const presenceStatus = React.useMemo(() => {
    if (!viewedUserId) return "offline" as const;
    return getPresenceStatus(viewedUserId);
  }, [getPresenceStatus, viewedUserId]);

  const lastActiveQuery = useUserLastActive(viewedUserId);
  const lastActiveLabel = React.useMemo(() => {
    if (presenceStatus === "online") return "Online";

    const label = formatTimeAgo(lastActiveQuery.data ?? null);
    if (!label) return null;
    if (label === "Just now") return "Online";
    return `Active ${label}`;
  }, [lastActiveQuery.data, presenceStatus]);
  ...
  if (isError || !profile) {
    return (
      <div className="flex flex-1 flex-col px-[var(--page-pad-x)] py-12">
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-1 flex-col pb-2">
      {/* Header */}
      <div className="relative h-80 w-full shrink-0">
        ...
        <div className="absolute left-0 top-0 z-20 flex w-full items-center justify-between p-[var(--page-pad-x)] pt-12">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex h-11 w-11 items-center justify-center rounded-full bg-black/20 text-white backdrop-blur-md transition-colors hover:bg-black/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-black/20"
            aria-label="Back"
          >
      ...
      {/* Avatar + core info */}
      <div className="relative z-10 -mt-20 px-[var(--page-pad-x)]">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <VerifiedBadge
                    ...
                    type={(profile.verifiedType as any) ?? null}
                    label={profile.verifiedLabel}
                    verifiedAt={profile.verifiedAt}
                    org={profile.verifiedByOrg}
                    className="mt-1 inline-flex items-center justify-center rounded-full p-1 text-white/90 hover:text-white"
                  />
                </div>
                {handle ? (
      ...
      {/* Stats */}
      <div className="flex justify-between px-[var(--page-pad-x)] py-6">
        <button
          type="button"
