...
export const useSuggestedPeople = () => {
  const { user } = useAuth();
  const viewerKey = user?.id ?? "__anon__";
  const queryClient = useQueryClient();

  const query = useQuery<SuggestedPerson[]>({
    queryKey: ["suggested-people", viewerKey],
    enabled: true,
    staleTime: 1000 * 60 * 5,
    gcTime: 1000 * 60 * 30,
    queryFn: async () => {
      const viewerId = user?.id ?? null;
      const dismissed = new Set(loadDismissedSuggestedPeople(viewerKey));
      ...
      const withAvatars: SuggestedPerson[] = await Promise.all(
        rawPeople.map(async (person) => {
          ...
          return {
            ...
            verifiedType: person.verifiedType ?? null,
            verifiedLabel: person.verifiedLabel ?? null,
            verifiedAt: person.verifiedAt ?? null,
            verifiedByOrg: person.verifiedByOrg ?? null,
          };
        }),
      );
