...
export const PeopleResultRow: React.FC<{
  ...
  if (variant === "compact") {
    ...
    return (
      <div
        className={`flex items-center justify-between gap-3 rounded-2xl border border-border bg-card/70 px-3 py-2 hover:bg-card ${
          className ?? ""
        }`}
      >
  ...
  return (
    <div
      className={`flex items-center justify-between gap-3 rounded-2xl border border-border bg-card/80 px-3 py-2 hover:bg-card/80 ${
        className ?? ""
      }`}
    >
      <Link to={profileHref} className="flex min-w-0 flex-1 items-center gap-3">
