...
import { getPublicSettingNumber } from "@/lib/settings/publicSettingsStore";
...
function getPageSize(): number {
  // Public setting; defaults match previous hard-coded behavior.
  const v = getPublicSettingNumber("ux.search.page_size", 20);
  return Math.max(1, Math.floor(v));
}

function getBatchSyncLimit(): number {
  const v = getPublicSettingNumber("ux.search.batch_sync_limit", 5);
  return Math.max(0, Math.floor(v));
}

function getSearchTimeoutMs(): number {
  const v = getPublicSettingNumber("ops.search.timeout_ms", 20_000);
  return Math.max(1_000, Math.floor(v));
}
...
export const searchTitles = async (params: {
  ...
  if (filteredExternal.length && (await isSessionAvailable())) {
    const BATCH_SYNC_LIMIT = getBatchSyncLimit();
    const timeoutMs = getSearchTimeoutMs();
