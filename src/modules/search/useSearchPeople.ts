...
type PublicProfileRow = ProfilePublicRow;
...
export const useSearchPeople = (query: string) => {
  ...
  return useQuery<PeopleSearchResult[]>({
    ...
    queryFn: async ({ signal }) => {
      ...
      // Resolve avatar storage paths into signed urls (cached) so lists render consistently.
      return Promise.all(
        baseResults.map(async (row) => ({
          ...row,
