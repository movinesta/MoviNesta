...
const DiaryTimelineTab: React.FC<DiaryTimelineTabProps> = ({
  ...
  if (!items.length) {
    return (
      <div className="flex min-h-[40vh] items-center justify-center px-4">
        <div className="max-w-sm rounded-2xl border border-border bg-card/80 p-5 text-center text-xs text-muted-foreground shadow-sm">
        ...
        components={{
          List: React.forwardRef<HTMLOListElement, React.HTMLAttributes<HTMLOListElement>>(
            function List(props, ref) {
              return <ol ref={ref} className="space-y-2" {...props} />;
            },
          ) as any,
        }}
        itemContent={(index, item) => {
          ...
          return (
            <li key={item.id} className="pb-1">
              <article className="soft-row-card flex gap-3 row-pad">
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-1.5">
                  ...
                  <div className="min-w-0">
                    {titleUrl ? (
                      <Link
                        to={titleUrl}
                        className="line-clamp-2 text-sm font-medium text-foreground hover:underline focus-visible:outline-none"
                      >
                        {item.title ?? "Untitled"}
                        {item.year ? (
