...
const AppShell: React.FC = () => {
  ...
  const shellContentClassName =
    isConversationRoute || isAssistantRoute
      ? "relative z-10 mx-auto flex min-h-[100dvh] w-full max-w-5xl flex-1 flex-col px-0 pt-0 pb-0"
      : isSwipeRoute
        ? // Swipe route: allow full-bleed content but still clear the bottom nav.
          "relative z-10 mx-auto flex w-full max-w-5xl flex-1 flex-col px-0 pt-0 pb-[calc(var(--app-bottom-nav)+env(safe-area-inset-bottom))]"
        : // Default layout (other tabs): keep padding so content clears bottom nav.
          "relative z-10 mx-auto flex w-full max-w-5xl flex-1 flex-col px-[var(--page-pad-x)] pb-[calc(var(--app-bottom-nav)+env(safe-area-inset-bottom))] pt-0";

  const bottomNavInnerClassName =
    isConversationRoute || isAssistantRoute
      ? "flex items-center justify-between gap-1 px-3 py-1.5"
      : "mx-auto flex max-w-5xl items-center justify-between gap-1 px-[var(--page-pad-x)] py-1.5 pb-[calc(env(safe-area-inset-bottom)+0.5rem)]";
  ...
  return (
    <div className="relative flex min-h-[100dvh] flex-col bg-background text-foreground overflow-x-hidden">
