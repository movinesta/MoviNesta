...
const Row: React.FC<{
  ...
  <button
    type="button"
    onClick={onClick}
    className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
  >
...
const CreateActionSheet: React.FC<CreateActionSheetProps> = ({ open, onOpenChange }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={
          "left-0 right-0 top-auto bottom-0 w-full max-w-none translate-x-0 translate-y-0 rounded-t-3xl rounded-b-none border border-border bg-card p-4"
        }
      >
        <div className="mx-auto mb-2 h-1.5 w-12 rounded-full bg-muted" aria-hidden />
        <DialogHeader className="pb-1">
          <DialogTitle className="text-center text-base text-foreground">Create</DialogTitle>
        </DialogHeader>

        <div className="mt-2 space-y-1">
          <Row
            icon={<Plus className="h-5 w-5 text-muted-foreground" aria-hidden />}
            title="Log a movie"
            description="Pick a title and add it to your diary"
            onClick={() => go("/search")}
          />
          <Row
            icon={<Star className="h-5 w-5 text-muted-foreground" aria-hidden />}
            title="Write a review"
            description="Find a title and leave your thoughts"
            onClick={() => go("/search")}
          />
          <Row
            icon={<BookmarkPlus className="h-5 w-5 text-muted-foreground" aria-hidden />}
            title="Create a highlight"
            description="Make a new list (Top movies, etc.)"
            onClick={() => go("/me?createHighlight=1")}
          />
          <Row
            icon={<Sparkles className="h-5 w-5 text-muted-foreground" aria-hidden />}
            title="Discover"
