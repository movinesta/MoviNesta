...
export const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-20 flex h-14 items-center justify-between gap-3 rounded-2xl border border-mn-border-subtle/80 bg-gradient-to-r from-mn-bg/90 via-mn-bg/95 to-mn-bg/90 px-4 shadow-mn-card backdrop-blur">
      <div className="flex min-w-0 items-center gap-3">
        <div className="flex min-w-0 items-center gap-2 truncate">
          <span className="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-mn-bg-elevated/80 p-1 shadow-mn-soft">
            <img
              src="/MoviNesta/src/assets/brand/movinesta-logo-neon.png"
              alt="MoviNesta"
              className="h-5 w-5 rounded-full object-contain"
            />
          </span>
          <div className="min-w-0 leading-tight">
            <p className="truncate text-lg font-semibold text-mn-text-primary">
              {title}
            </p>
            {description && (
              <p className="truncate text-[11px] text-mn-text-muted">
                {description}
              </p>
            )}
          </div>
